...
def yield_bonds(R: Reaction, TC: TopoCoord, resid_mapper):
    ...
    for bondrec in R.bonds:
        ...
        if nreactants == 1:
            in_product_resids = [
                resid_mapper[0][in_reactant_resids[x]] for x in [0, 1]
            ]
        else:
            in_product_resids = [
                resid_mapper[x][in_reactant_resids[x]] for x in [0, 1]
            ]
        atom_idx = [
            TC.get_gro_attribute_by_attributes(
                'globalIdx', {
                    'resNum': in_product_resids[x],
                    'atomName': atom_names[x]
                }
            ) for x in [0, 1]
        ]
        bystander_resids, bystander_resnames, bystander_atomidx, bystander_atomnames = TC.get_bystanders(
            atom_idx
        )
        oneaway_resids, oneaway_resnames, oneaway_atomidx, onewaway_atomnames = TC.get_oneaways(
            atom_idx
        )
        yield ReactionBond(
            atom_idx, in_product_resids, order, bystander_resids,
            bystander_atomidx, oneaway_resids, oneaway_atomidx
        )
...
class Molecule:
    ...
    def New(cls, mol_name, generator: Reaction, molrec={}):
        ...
        logger.debug(
            f'{M.name} stereoisomers: {[s.name for s in M.stereoisomers.values()]}'
        )
    ...
    def update_zrecs(self, zrecs, moldict):
        ...
        def replace_if_greater(rec, D, matchattr=[], maxattr=[]):
            if not matchattr or not maxattr:
                return
            for r in D:
        ...
        for zr in zrecs:
            ...
            found = replace_if_greater(
                zr, target.zrecs, matchattr=['resid', 'atom'], maxattr=['z']
            )
            if not found:
    ...
    def create_new_stereoisomers(self):
        ...
        """
        if self.generator:
            return    # we only consider stereoisomers on monomers
        if not self.stereocenters:
            return
    ...
    def initialize_monomer_grx_attributes(self):
        ...
        for s in self.symmetry_relateds:
            ...
            for atomName in s:
                ...
                TC.set_gro_attribute_by_attributes(
                    'sea_idx', sea_idx, {'atomName': atomName}
                )
            sea_idx += 1
        ...
        for i, j in pairs:
            if i < j:
                iname = TC.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': i}
                )
                jname = TC.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': j}
                )
                if TC.are_bonded(
                    i, j
                ) and iname.startswith('C') and jname.startswith('C'):
    ...
    def previously_parameterized(self):
        ...
        rval = True
        for ext in ['gro']:
            rval = rval and pfs.exists(
                os.path.join('molecules/parameterized', f'{self.name}.{ext}')
            )
        return rval
    ...
    def relax(self, relax_dict):
        ...
        logger.info(
            f'In vacuo equilibration of {self.name}.gro for {nsteps} steps at {temperature} K'
        )
        self.TopoCoord.grompp_and_mdrun(
            out=deffnm, mdp=mdp_prefix, boxSize=boxsize
        )
    ...
    def generate(self, outname='', available_molecules={}, **kwargs):
        ...
        if self.generator:
            R = self.generator
            if R.stage in [
                reaction_stage.cure, reaction_stage.param, reaction_stage.cap
            ]:
                do_parameterization = True
            self.TopoCoord = TopoCoord()
            logger.debug(
                f'Using reaction {R.name} ({str(R.stage)}) to generate {self.name} parent {self.parentname}'
            )
            isf = 'mol2'
            resid_mapper = []
            for ri in R.reactants.values():
                ...
                new_reactant.TopoCoord.write_mol2(
                    filename=f'{self.name}-reactant{ri}-prebonding.mol2',
                    molname=self.name
                )
                ...
                resid_mapper.append(
                    {
                        ...
                        for k, v in zip(
                            range(1, rnr +
                                  1), range(1 + shifts[2], 1 + rnr + shifts[2])
                        )
                    }
                )
            ...
            self.TopoCoord.set_gro_attribute(
                'sea_idx', -1
            )    # turn off symmetry-equivalence for multimers
            ...
            for isf in input_structure_formats:
                if pfs.exists(f'molecules/inputs/{self.name}.{isf}'):
                    logger.debug(
                        f'Using input molecules/inputs/{self.name}.{isf} as a generator'
                    )
                    pfs.checkout(f'molecules/inputs/{self.name}.{isf}')
                    break
        ...
        if do_parameterization:
            ...
            if self.name != self.parentname:
                logger.info(
                    f'Built {self.name} using topology of {self.parentname}; copying {self.parentname}.top to {self.name}.top'
                )
                self.load_top_gro(
                    f'{self.parentname}.top',
                    f'{self.name}.gro',
                    wrap_coords=False
                )
    ...
    def prepare_new_bonds(self, available_molecules={}):
        ...
        for bondrec in R.bonds:
            ...
            in_reactant_resids = [x['resid'] for x in atomrecs]
            if reactant_keys[0] == reactant_keys[
                1]:    # this is an intraresidue bond
                reactant_names = [R.reactants[reactant_keys[0]]]
            else:
                reactant_names = [R.reactants[x] for x in reactant_keys]
            reactant_sequences = [
                available_molecules[x].sequence for x in reactant_names
            ]
            product_sequence = []
            ...
            in_product_resids = [
                in_reactant_resids[x] + sequence_residue_idx_origins[x]
                for x in [0, 1]
            ]
            # logger.debug(f'in_product_resids {in_product_resids}')
            in_product_resnames = [
                product_sequence[in_product_resids[x] - 1] for x in [0, 1]
            ]
            atom_idx = [
                TC.get_gro_attribute_by_attributes(
                    'globalIdx', {
            ...
            oneaway_resids, oneaway_resnames, oneaway_atomidx, oneaway_atomnames = TC.get_oneaways(
                atom_idx
            )
            ...
            self.bond_templates.append(
                BondTemplate(
                    atom_names, in_product_resnames, intraresidue, order,
                    bystander_resnames, bystander_atomnames, oneaway_resnames,
                    oneaway_atomnames
                )
            )

    def idx_mappers(
        self, otherTC: TopoCoord, other_bond, bystanders, oneaways,
        uniq_atom_idx: set
    ):
        ...
        i_resName, i_resNum, i_atomName = otherTC.get_gro_attribute_by_attributes(
            ['resName', 'resNum', 'atomName'], {'globalIdx': i_idx}
        )
        j_resName, j_resNum, j_atomName = otherTC.get_gro_attribute_by_attributes(
            ['resName', 'resNum', 'atomName'], {'globalIdx': j_idx}
        )
        logger.debug(
            f'i_idx {i_idx} i_resName {i_resName} i_resNum {i_resNum} i_atomName {i_atomName}'
        )
        logger.debug(
            f'j_idx {j_idx} j_resName {j_resName} j_resNum {j_resNum} j_atomName {j_atomName}'
        )
        ...
        assert len(
            ij
        ) == 2, f'Mappers using template {self.name} unable to map from instance bond {i_resName}-{i_resNum}-{i_atomName}---{j_resName}-{j_resNum}-{j_atomName}'
        ...
        assert all(
            [
                len(inst_bystander_resids[x]) == len(temp_bystander_resids[x])
                for x in [0, 1]
            ]
        ), f'Error: bystander count mismatch'
        ...
        for inst, temp in zip(
            [
                ...
                *inst_bystander_resids[1], *inst_oneaway_resids
            ], [
                *temp_resids, *temp_bystander_resids[0],
                *temp_bystander_resids[1], *temp_oneaway_resids
            ]
        ):
            if inst and temp:    # None's in the bystander lists and oneaways lists should be ignored
                ...
                idf = instdf[instdf['resNum'] == inst][['globalIdx',
                                                        'atomName']].copy()
                # logger.debug(f'idf res {inst}:\n{idf.to_string()}')
                tdf = tempdf[tempdf['resNum'] == temp][['globalIdx',
                                                        'atomName']].copy()
        ...
        assert len(inst2temp) == len(
            temp2inst
        ), f'Error: could not establish two-way dict of atom globalIdx'
    ...
    def get_angles_dihedrals(self, bond):
        ...
        ad = d[((d.ai == ai) & (d.aj == aj)) | ((d.ai == aj) & (d.aj == ai)) |
               ((d.aj == ai) & (d.ak == aj)) | ((d.aj == aj) &
                                                (d.ak == ai))].copy()
        d = self.TopoCoord.Topology.D['dihedrals']
        td = d[((d.ai == ai) & (d.aj == aj)) | ((d.ai == aj) & (d.aj == ai)) |
               ((d.aj == ai) & (d.ak == aj)) | ((d.aj == aj) & (d.ak == ai)) |
               ((d.ak == ai) & (d.al == aj)) | ((d.ak == aj) &
                                                (d.al == ai))].copy()
        check = True
    ...
    def read_gro_attributes(self, grxfilename, attribute_list=[]):
        ...
        """
        self.TopoCoord.read_gro_attributes(
            grxfilename, attribute_list=attribute_list
        )
    ...
    def make_bonds(self, bdf: pd.DataFrame, moldict, stage):
        ...
        for i, r in bdf.iterrows():
            ...
            if r.ri != r.rj:
                ...
                hxi, hxj = self.transrot(
                    r.ai, r.ri, r.aj, r.rj, connected_resids=resid_sets[1]
                )
        ...
        TC.update_topology_and_coordinates(
            bdf,
            moldict,
            explicit_sacH=explicit_sacrificial_Hs,
            template_source=template_source
        )
        self.initialize_molecule_cycles()

    def transrot(
        self, at_idx, at_resid, from_idx, from_resid, connected_resids=[]
    ):
        ...
        logger.debug(
            f'rotating/translating {bresids} ({BTC.Coordinates.A.shape[0]})'
        )
        assert self.TopoCoord.Coordinates.A.shape[0] == (
            NONROT + BTC.Coordinates.A.shape[0]
        )
        ...
        myHpartners = {
            ...
            for k, v in zip(
                mypartners, [
                    C[C['globalIdx'] == i]['atomName'].values[0]
                    for i in mypartners
                ]
            ) if v.startswith('H')
        }
        ...
        myHighestH = {
            k: v
            for k, v in myHpartners.items() if v == max(
                [k for k in myHpartners.values()],
                key=lambda x:
        ...
        otHighestH = {
            k: v
            for k, v in otHpartners.items() if v == max(
                [k for k in otHpartners.values()],
                key=lambda x:
                int(x.split('H')[1] if x.split('H')[1] != '' else '0')
            )
        }
        ...
        assert len(
            myHpartners
        ) > 0, f'Error: atom {at_idx} does not have a deletable H atom!'
        assert len(
            otHpartners
        ) > 0, f'Error: atom {from_idx} does not have a deletable H atom!'

        Ri = TC.get_R(at_idx)
        Rj = TC.get_R(from_idx)
        overall_maximum = (-1.e9, -1, -1)
        coord_trials = {}
        for myH, myHnm in myHpartners.items(
        ):    # keys are globalIdx's, values are names
            ...
            for otH, otHnm in otHpartners.items():
                ...
                c = np.dot(Rkj, Rih)
                v = np.array(
                    [[0, -cp[2], cp[1]], [cp[2], 0, -cp[0]], [-cp[1], cp[0], 0]]
                )
                v2 = np.dot(v, v)
                ...
                minD = TC.minimum_distance(
                    coord_trials[myH][otH],
                    self_excludes=[myH],
                    other_excludes=[otH]
                )
    ...
    def atoms_w_same_attribute_as(
        self, find_dict={}, same_attribute='', return_attribute=''
    ):
        ...
        :rtype: list
        """
        att_val = self.TopoCoord.get_gro_attribute_by_attributes(
            same_attribute, find_dict
        )
        return self.TopoCoord.get_gro_attributelist_by_attributes(
            return_attribute, {same_attribute: att_val}
        )
    ...
    def flip_stereocenter(self, idx):
        ...
        if len(ligand_idx) != 4:
            logger.debug(
                f'Atom {idx} cannot be a stereocenter; it only has {len(ligand_idx)} ligands.'
            )
            return
        ...
        R = I + v + v2 / (1. + c)
        dfrotate(adf, R)
        A.loc[A['globalIdx'].isin(aset),
              ['posX', 'posY', 'posZ']] = adf[['posX', 'posY', 'posZ']]
        cp = np.cross(rOb, rOa)
        v = np.array(
            [[0, -cp[2], cp[1]], [cp[2], 0, -cp[0]], [-cp[1], cp[0], 0]]
        )
        ...
        R = I + v + v2 / (1. + c)
        dfrotate(bdf, R)
        A.loc[A['globalIdx'].isin(bset),
              ['posX', 'posY', 'posZ']] = bdf[['posX', 'posY', 'posZ']]
    ...
    def rotate_bond(self, a, b, deg):
        ...
        A.loc[A['globalIdx'].isin(branchidx),
              ['posX', 'posY', 'posZ']] = bdf[['posX', 'posY', 'posZ']]
        TC.translate(O)
    ...
    def generate_stereoisomers(self):
        ...
        :rtype: None
        """
        if self.TopoCoord.Topology.D['atoms'].shape[0] == 0:
            return    # self has not yet acquired topology/coordinates
        if len(self.stereoisomers) == 0:
            return
        flip = [[0, 1] for _ in range(len(self.stereocenters))]
        st_idx = [
            self.TopoCoord.get_gro_attribute_by_attributes(
                'globalIdx', {'atomName': n}
            ) for n in self.stereocenters
        ]
        P = product(*flip)
        ...
        for p in P:
            ...
            if not si_name in self.stereoisomers:
                logger.debug(
                    f'{si_name} not found in dict of stereoisomers of {self.name}'
                )
            logger.debug(
                f'Stereocenter sequence {p} generates stereoisomer {si_name}'
            )
    ...
    def generate_conformers(self):
        ...
        if generator == 'obabel' and not minimize:
            logger.debug(
                f'Confomers generated by obabel should be energy-minimized.  Indicate "mimimize: True" in the confomers directive for {self.name}'
            )
            minimize = True
        if not generator:
            return
        logger.info(
            f'Generating {self.nconformers*(1+len(self.stereoisomers))} conformers for {self.name}'
        )
        ...
        for gro in gronames:
            ...
            if generator['name'] == 'obabel':
                ...
                c = Command(
                    f'obabel -igro {gro}.gro -O {compfile} --conformer --nconf {self.nconformers} --writeconformers'
                )
                out, err = c.run()
                ...
                c = Command(
                    f'split -d -n {nd} -l {lpf} {compfile} {pfx} --additional-suffix=".gro"'
                )
                out, err = c.run()
                ...
                TC = self.TopoCoord if gro == self.name else self.stereoisomers[
                    gro].TopoCoord
                TC.vacuum_simulate(
                    outname=f'{compfile}', nsamples=cd['count'], params=params
                )
                gro_from_trr(compfile, nzero=nd, outpfx=pfx, b=begin_at)
            # os.remove(f'{gro}-confs.gro')
            fmt = r'{A}{B:0' + str(
                nd
            ) + r'd}'    # the trjconv command in gro_from_trr must generate these files
            cfnl = [fmt.format(A=pfx, B=x) for x in range(self.nconformers)]
            for mname in cfnl:
                assert os.path.exists(
                    f'{mname}.gro'
                ), f'Error: Conformer coordinates file {mname}.gro not found'
...
def generate_stereo_reactions(RL: ReactionList, MD: MoleculeDict):
    ...
    terminal_reactions = []
    for R in RL:
        if R.stage not in [reaction_stage.param, reaction_stage.build]:
            continue
        Prod = MD[R.product]
        logger.debug(f'Stereos for {R.name} ({str(R.stage)})')
        reactant_stereoisomers = {
            k: [r] + list(MD[r].stereoisomers.keys())
            for k, r in R.reactants.items()
        }
        for k, v in reactant_stereoisomers.items():
            assert all(
                [m in MD for m in v]
            )    # all stereoisomers must be in the dict of molecules
...
def generate_symmetry_reactions(RL: ReactionList, MD: MoleculeDict):
    ...
    for R in RL:
        if R.stage not in [
            reaction_stage.param, reaction_stage.cure, reaction_stage.cap
        ]:
            continue
        Prod = MD[R.product]
        logger.debug(
            f'Symmetry versions for {R.name} ({str(R.stage)})\n{str(R)}'
        )
        # thisR_extra_reactions=[]
        # thisR_extra_molecules={}
        # logger.debug(f'  Product {R.product} resname sequence {prod_seq_resn}')
        sra_by_reactant = {
            k: MD[rname].symmetry_relateds
            for k, rname in R.reactants.items()
        }
        ...
        for atom_key, atom_rec in R.atoms.items():
            ...
            if art in sra_by_reactant:
                ...
                if len(sra_by_reactant[art]) == 0:
                    sra_by_reactant[art] = [[target_atom_name]]
        ...
        for P in olist[1:]:
            ...
            for rR in [x for x in RL if R.product in x.reactants.values()]:
                reactantKey = list(rR.reactants.keys())[list(
                    rR.reactants.values()
                ).index(R.product)]
                logger.debug(
                    f'  New product {newR.product} must replace reactant {reactantKey}:{R.product} in {rR.name}'
                )
                nooR = deepcopy(rR)
                nooR.stage = rR.stage
                ...
                for naK, naRec in {
                    k: v
                    for k, v in nooR.atoms.items()
                    if v['reactant'] == reactantKey
                }.items():
                    na_resid = naRec[
                        'resid']    # resid of reactant atom in target reactant
                    na_name = naRec['atom']
                    for p in P:
                        oaK, oa_name = p
                        oaRec = R.atoms[oaK]
                        ...
                        oa_resid_in_o_product = reactant_resid_to_presid(
                            R, oa_reactatnName, oa_resid, RL
                        )
