...
class RuntimeLibrary:
    ...
    def get_example_names(self):
        ...
        assert os.path.exists(depot) and os.path.isdir(
            depot
        ), f'Depot not found or not a directory -- your installation is corrupt.'
        owd = os.getcwd()
        os.chdir(depot)
    ...
    def user(cls, pathname='.'):
        ...
        assert os.path.exists(
            pathname
        ), f'Cannot find {pathname} in {os.getcwd()}'
    ...
    def checkin(self, filename, overwrite=False):
        ...
        if not os.path.exists(basefilename):
            logger.debug(
                f'{basefilename} not found in {os.getcwd()}. No check-in performed.'
            )
            return False
        ...
        if os.path.exists(fullfilename):
            if overwrite:
                ...
                logger.debug(
                    f'{filename} already exists in system library. No check-in performed.'
                )
        else:
            shutil.copyfile(basefilename, fullfilename)
        return True
    ...
    def checkout(self, filename, searchpath=[], altpath=[]):
        ...
        if os.path.exists(fullfilename):
            shutil.copyfile(
                fullfilename, os.path.join(os.getcwd(), basefilename)
            )
            return True
        else:
    ...
    def _next_project_dir(self, projdir='next', reProject=False, prefix='proj-'):
        ...
        if not projdir == 'next':    # explicit project directory is named
            ...
            if not reProject or lastprojdir == '':    # this is a fresh project
                ...
                logger.info(
                    f'Restarting project in {self.projPath} (latest project)'
                )
...
def checkout(filename, altpath=[]):
    ...
    :rtype: bool
    """
    if _PFS_.userlibrary and _PFS_.userlibrary.checkout(
