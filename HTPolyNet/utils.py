...
def density_from_gro(gro, mollib='./lib/molecules/parameterized', units='SI'):
    ...
    templates = {
        x:
            TopoCoord.from_top_gro(
                os.path.join(mollib, f'{x}.top'),
                os.path.join(mollib, f'{x}.gro')
            )
        for x in resnames
    }
    mass = 0.0
...
_system_dirs = [
    'densification', 'precure', r'iter-{iter:d}', 'capping', 'postcure'
]
_postsim_dirs = ['anneal', 'equilibrate']
_measurement_dirs = ['Tg', 'E']
_md_ensembles = {
    'nvt': ['Temperature', 'Potential'],
    'npt': ['Temperature', 'Potential', 'Density'],
    'default': ['Temperature', 'Potential']
}
_indir_pfx = {}
_indir_pfx['densification'] = [
    r'densified-{ens:s}', r'densified-repeat-{repeat:d}-{ens:s}'
]
_indir_pfx['precure'] = [
    r'preequilibration-{ens:s}', r'annealed', r'postequilibration-{ens:s}'
]
_indir_pfx['iter-n'] = [
    r'1-cure_drag-stage-{stage:d}-{ens:s}',
    r'3-cure_relax-stage-{stage:d}-{ens:s}', r'4-cure_equilibrate-{ens:s}'
]
_indir_pfx['capping'] = [
    r'7-cap_relax-stage-{stage:d}-{ens:s}', r'8-cap_equilibrate-{ens:s}'
]
_indir_pfx['postcure'] = [
    r'preequilibration-{ens:s}', r'annealed', r'postequilibration-{ens:s}'
]
...
def density_evolution(proj_dir):
    ...
    for subd in _system_dirs:
        ...
        if subd == r'iter-{iter:d}' or subd == 'capping' or subd == 'densification':
            ...
            # print(iter_mark0)
            iter = 1
            iter_subd_rel = subd if subd in ['capping', 'densification'
                                            ] else subd.format(iter=iter)
            ...
            while os.path.exists(iter_subd):
                ...
                if subd == r'iter-{iter:d}':
                    if os.path.exists(
                        os.path.join(iter_subd, '2-cure_update-bonds.csv')
                    ):
                        bdf = pd.read_csv(
                            os.path.join(iter_subd, '2-cure_update-bonds.csv'),
                            sep='\s+',
                        ...
                        if os.path.exists(
                            os.path.join(iter_subd, '6-cap_update-bonds.csv')
                        ):
                            bdf = pd.read_csv(
                                os.path.join(
                                    iter_subd, '6-cap_update-bonds.csv'
                                ),
                                sep='\s+',
                                header=0,
                                index_col=None
                ...
                for pfx in _indir_pfx[dirkey]:
                    ...
                    if r'stage' in pfx or r'repeat' in pfx:
                        ...
                        stg_present = any(
                            [
                                os.path.exists(
                                    os.path.join(
                                        iter_subd,
                                        pfx.format(stage=stg, repeat=stg, ens=x)
                                    ) + r'.edr'
                                ) for x in _md_ensembles
                            ]
                        )
                        # print(f'{iter_subd} {stg} {pfx} stg_present {stg_present}')
                        while stg_present:
                            for ens, qtys in _md_ensembles.items():
                                edr_pfx = os.path.join(
                                    iter_subd,
                                    pfx.format(stage=stg, repeat=stg, ens=ens)
                                )
                                gro = edr_pfx + r'.gro'
                                ...
                                if os.path.exists(edr_pfx + r'.edr'):
                                    ...
                                    df, xshift = _concat_from_edr(
                                        df,
                                        edr_pfx,
                                        qtys,
                                        add=[('nbonds', nbonds)],
                                        add_if_missing=[('Density', density)]
                                    )
                                ...
                                [
                                    os.path.exists(
                                        os.path.join(
                                            iter_subd,
                                            pfx.
                                            format(stage=stg, repeat=stg, ens=x)
                                        ) + r'.edr'
                                    ) for x in _md_ensembles
                                ]
                            )
                        ...
                        eq_present = any(
                            [
                                os.path.exists(
                                    os.path.join(iter_subd, pfx.format(ens=x)) +
                                    r'.edr'
                                ) for x in _md_ensembles
                            ]
                        )
                ...
                iter_subd = os.path.join(
                    sysd, iter_subd_rel
                ) if r'iter' in subd else 'I_BET_THIS_FILE_DNE'
                        ...
                        if os.path.exists(edr_pfx + r'.edr'):
                            ...
                            if ens == 'nvt' and os.path.exists(gro):
                                density = density_from_gro(
                                    gro,
                                    os.path.join(
                                        proj_dir, 'molecules/parameterized'
                                    )
                                )
                            df, xshift = _concat_from_edr(
                                df,
