...
class TopoCoord:
    ...
    def make_bonds(self, pairs, explicit_sacH={}):
        ...
        :rtype: list
        """
        idx_to_ignore = self.Coordinates.find_sacrificial_H(
            pairs, self.Topology, explicit_sacH=explicit_sacH
        )
        ...
        idx_to_delete = self.Coordinates.find_sacrificial_H(
            pairs, self.Topology, explicit_sacH=explicit_sacH, rename=rename
        )
    ...
    def count_H(self, idx):
        ...
        aneighnames = [
            self.get_gro_attribute_by_attributes('atomName', {'globalIdx': y})
            for y in aneigh
        ]
    ...
    def map_from_templates(self, bdf, moldict, overcharge_threshhold=0.1):
        ...
        logger.debug(
            f'Must compensate for an overcharge of {premapping_total_charge:.4f}'
        )
        mapped_inst_atoms = []
        for b in bdf.itertuples():
            ...
            order = b.order
            names = [
                self.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': x}
                ) for x in bb
            ]
            resnames = [
                self.get_gro_attribute_by_attributes(
            ...
            resids = [
                self.get_gro_attribute_by_attributes('resNum', {'globalIdx': x})
                for x in bb
            ]
            ...
            if product_name in moldict:
                ...
                if P.is_reactant:
                    logger.debug(
                        f'{P.name} is a reactant; updating \"reactantName\" attributes of {bb}'
                    )
                    ...
                    for j in bb:
                        self.set_gro_attribute_by_attributes(
                            'reactantName', product_name, {'globalIdx': j}
                        )
                else:
                    logger.debug(
                        f'{P.name} is not a reactant; no update of \"reactantName\" attributes'
                    )
            bystander_resids, bystander_resnames, bystander_atomidx, bystander_atomnames = self.get_bystanders(
                bb
            )
            oneaway_resids, oneaway_resnames, oneaway_atomidx, oneaway_atomnames = self.get_oneaways(
                bb
            )
            intraresidue = resids[0] == resids[1]
            BT = BondTemplate(
                names, resnames, intraresidue, order, bystander_resnames,
                bystander_atomnames, oneaway_resnames, oneaway_atomnames
            )
            RB = ReactionBond(
                bb, resids, order, bystander_resids, bystander_atomidx,
                oneaway_resids, oneaway_atomidx
            )
            ...
            assert d.shape[
                0
            ] == 1, f'Using {T.name} is sent inst-bond {i_idx}-{j_idx} which is claimed to map to {temp_i_idx}-{temp_j_idx}, but no such unique bond is found:\n{T.TopoCoord.Topology.D["bonds"].to_string()}'
            ''' check passed '''
            temp_angles, temp_dihedrals, temp_pairs = T.get_angles_dihedrals(
                (temp_i_idx, temp_j_idx)
            )
            logger.debug(
                f'Mapping {temp_angles.shape[0]} angles, {temp_dihedrals.shape[0]} dihedrals, and {temp_pairs.shape[0]} pairs from template {T.name}'
            )
            ...
            for df in [temp_angles, temp_dihedrals, temp_pairs]:
                for label in ['ai', 'aj', 'ak', 'al']:
                    if label in df:
                        uniq_temp_idx = uniq_temp_idx.union(
                            set(df[label].to_list())
                        )
            logger.debug(
                f'Template atom indexes that must be mapped: {uniq_temp_idx}'
            )
            # get the bidirectional instance<->template mapping dictionaries
            inst2temp, temp2inst = T.idx_mappers(
                self, RB.idx, RB.bystander_resids, RB.oneaway_resids,
                uniq_temp_idx
            )
            ...
            for temp_atom, inst_atom in temp2inst.items():
                assert inst_atom in atdf[
                    'nr'
                ].values, f'Error: mapped atom {inst_atom} not found in [ atoms ]'
                inst_type, inst_charge, inst_name, inst_resn, inst_rnam = atdf[
                    atdf['nr'] == inst_atom][[
                        'type', 'charge', 'atom', 'resnr', 'residue'
                    ]].values[0]
                temp_type, temp_charge, temp_name, temp_resn, temp_rnam = temp_atdf[
                    temp_atdf['nr'] == temp_atom][[
                        'type', 'charge', 'atom', 'resnr', 'residue'
                    ]].values[0]
                ...
                if inst_type != temp_type:
                    logger.debug(
                        f'changing type of inst atom {inst_atom} ({inst_resn} {inst_rnam} {inst_name}) from {inst_type} to {temp_type}'
                    )
                ...
                if inst_charge != temp_charge:
                    logger.debug(
                        f'charge {inst_atom} ({inst_resn} {inst_rnam} {inst_name}) from {inst_charge} to {temp_charge}'
                    )
            ...
            if need_new_bond_parameters:
                self.Topology.reset_override_from_type(
                    'bonds', 'bondtypes', inst_idx=(b.ai, b.aj)
                )
            ...
            self.Topology.D['angles'] = pd.concat(
                (d, inst_angles), ignore_index=True
            )
            ...
            if check:
                logger.error(
                    f'a {a} NAN in dihedrals\n{inst_dihedrals.to_string()}'
                )
                raise Exception
            # add new dihedrals to global topology
            d = self.Topology.D['dihedrals']
            self.Topology.D['dihedrals'] = pd.concat(
                (d, inst_dihedrals), ignore_index=True
            )
            ...
            if any(isin):
                for ii, jj in enumerate(isin):
                    if jj:
                        logger.error(
                            f'atom ai {temp_pairs.ai.iloc[ii]} not in temp2inst'
                        )
            ...
            if any(isin):
                for ii, jj in enumerate(isin):
                    if jj:
                        logger.error(
                            f'atom aj {temp_pairs.aj.iloc[ii]} not in temp2inst'
                        )
            ...
            #     logger.debug(ln)
            self.Topology.D['pairs'] = pd.concat(
                (d, temp_pairs), ignore_index=True
            )
            d = self.Topology.D['pairs']
        ...
        logger.debug(
            f'System overcharge after mapping: {self.Topology.total_charge():.4f}'
        )
        self.adjust_charges(
            atoms=mapped_inst_atoms,
            overcharge_threshhold=overcharge_threshhold,
            msg=f'overcharge magnitude exceeds {overcharge_threshhold}'
        )
    ...
    def update_topology_and_coordinates(
        self, bdf, template_dict={}, write_mapper_to=None, **kwargs
    ):
        ...
        if bdf.shape[0] > 0:
            ...
            idx_mapper = self.delete_atoms(
                idx_to_delete
            )    # will result in full reindexing
            ...
            for idx_pair in at_idx:
                for idx in idx_pair:
                    self.decrement_gro_attribute_by_attributes(
                        'z', {'globalIdx': idx}
                    )
                    self.increment_gro_attribute_by_attributes(
                        'nreactions', {'globalIdx': idx}
                    )
            if template_source == 'internal':
                logger.debug(f'calling map_from_templates')
                self.map_from_templates(
                    ri_bdf,
                    template_dict,
                    overcharge_threshhold=overcharge_threshhold
                )
            ...
            if write_mapper_to:
                tdf = pd.DataFrame(
                    {
                        'old': list(idx_mapper.keys()),
                        'new': list(idx_mapper.values())
                    }
                )
    ...
    def attenuate_bond_parameters(
        ...
        :type lengths: float
        """
        self.Topology.attenuate_bond_parameters(
            bonds,
            i,
            n,
    ...
    def attenuate_pair_parameters(self, pairdf, i, n, draglimit_nm=0.3):
        ...
        """
        self.Topology.attenuate_pair_parameters(
            pairdf, i, n, minimum_distance=draglimit_nm
        )
    ...
    def read_gro_attributes(self, grxfilename, attribute_list=[]):
        ...
        attributes_read = self.Coordinates.read_atomset_attributes(
            grxfilename, attributes=attribute_list
        )
    ...
    def set_gro_attribute_by_attributes(
        self, att_name, att_value, attribute_dict
    ):
    ...
    def get_gro_attributelist_by_attributes(
        self, attribute_list, attribute_dict
    ):
        ...
        :rtype: pd.DataFrame
        """
        return self.Coordinates.get_atoms_w_attribute(
    ...
    def minimum_distance(self, other, self_excludes=[], other_excludes=[]):
        ...
        :rtype: float
        """
        return self.Coordinates.minimum_distance(
            other.Coordinates,
            self_excludes=self_excludes,
            other_excludes=other_excludes
        )
    ...
    def adjust_charges(
        self, atoms=[], overcharge_threshhold=0.1, netcharge=0.0, msg=''
    ):
    ...
    def linkcell_initialize(self, cutoff, ncpu=1, force_repopulate=True):
        ...
        :type ncpu: int
        """
        self.Coordinates.linkcell_initialize(
            cutoff, ncpu=ncpu, populate=True, force_repopulate=force_repopulate
        )
    ...
    def inherit_grx_attributes_from_molecules(
        self,
        molecule_dict,
        initial_composition,
        globally_unique=GRX_GLOBALLY_UNIQUE,
        unset_defaults=GRX_UNSET_DEFAULTS,
        overall_default=0
    ):
        ...
        adf = self.Coordinates.A
        self.Coordinates.A = adf.drop(
            columns=[a for a in self.grxattr if a in adf]
        )

        logger.debug(
            f'{adf.shape[0]} atoms inheriting these attributes from molecular templates:'
        )
        logger.debug(f'    Attribute name     Default value   Globally unique?')
        for attname, defval, gu in zip(
            self.grxattr, unset_defaults, globally_unique
        ):
            ...
            for molecule_number in range(count):
                for i, k in enumerate(self.grxattr):
                    ...
                    if k == 'molecule':
                        ...
                        nuv = len(
                            list(
                                set([x for x in tra if x != unset_defaults[i]])
                            )
                        )
                        utra = []
                        for x in tra:
                            if globally_unique[i] and (
                                type(x) == int or type(x) == float
                            ):
                                xx = x + value_counts[k] if x != unset_defaults[
                                    i] else unset_defaults[i]
                            else:
                                xx = x
                            utra.append(xx)
                        value_counts[k] += nuv
    ...
    def write_mol2(self, filename, molname='', element_names_as_types=False):
        ...
        if 'mol2_bonds' in self.Topology.D:
            self.Coordinates.write_mol2(
                filename,
                molname=molname,
                bondsDF=self.Topology.D['mol2_bonds'],
                other_attributes=other_attributes
            )
        else:
            self.Coordinates.write_mol2(
                filename, molname=molname, other_attributes=other_attributes
            )
    ...
    def bondtest_df(self, df: pd.DataFrame, pbc=[1, 1, 1], show_piercings=True):
        ...
        for r in df.itertuples():
            result, dummy = self.bondtest(
                (r.ai, r.aj, r.r), pbc=pbc, show_piercings=show_piercings
            )
    ...
    def pierces_ring(self, i, j, pbc=[1, 1, 1], show_piercings=True):
        ...
        i_lcidx = self.get_gro_attribute_by_attributes(
            'linkcell_idx', {'globalIdx': i}
        )
        j_lcidx = self.get_gro_attribute_by_attributes(
            'linkcell_idx', {'globalIdx': j}
        )
        joint_idx = []
        for idx in LC.neighborlists[i_lcidx] + LC.neighborlists[j_lcidx] + [
            i_lcidx, j_lcidx
        ]:
        ...
        cycle_tags = list(
            set(
                adf[(adf['cycle'] != -1) &
                    (adf['linkcell_idx'].isin(joint_idx))]['cycle'].to_list()
            )
        )
    ...
    def makes_shortcircuit(self, i, j):
        ...
        :rtype: bool
        """
        i_resName, i_resNum, i_atomName, i_molNum = self.get_gro_attribute_by_attributes(
            ['resName', 'resNum', 'atomName', 'molecule'], {'globalIdx': i}
        )
        j_resName, j_resNum, j_atomName, j_molNum = self.get_gro_attribute_by_attributes(
            ['resName', 'resNum', 'atomName', 'molecule'], {'globalIdx': j}
        )
        '''
        ...
        R1DF = R1DF[[
            (not (b.startswith('H') or b.startswith('h')))
            for b in R1DF['atomName']
        ]]
        R2DF = ADF[ADF['molecule'] == j_molNum]
        R2DF = R2DF[[
            (not (b.startswith('H') or b.startswith('h')))
            for b in R2DF['atomName']
        ]]
        ...
        '''
        for i, j in product(
            R1DF['globalIdx'].to_list(), R2DF['globalIdx'].to_list()
        ):
    ...
    def reset_grx_attributes_from_idx_list(self, list_name):
        ...
        for i, c in enumerate(self.idx_lists[list_name]):
            for j, x in enumerate(c):
                self.set_gro_attribute_by_attributes(
                    list_name, i, {'globalIdx': x}
                )
                self.set_gro_attribute_by_attributes(
                    f'{list_name}_idx', j, {'globalIdx': x}
                )
    ...
    def reset_idx_list_from_grx_attributes(self, list_name):
        ...
        if tmp_dict:
            ...
            assert all(
                consec_test
            ), f'{list_name} reset_idx_list for group attribute {list_name} has non-consecutive integer keys -- bug\n{[a in tmp_dict for a in range(len(tmp_dict))]}'
    ...
    def chainlist_update(self, new_bond_recs, msg=''):
        ...
        for b in new_bond_recs:
            ...
            ar = self.get_gro_attribute_by_attributes(
                'resNum', {'globalIdx': aidx}
            )
            br = self.get_gro_attribute_by_attributes(
                'resNum', {'globalIdx': bidx}
            )
            if ar == br:
            ...
            bc = self.get_gro_attribute_by_attributes(
                'chain', {'globalIdx': bidx}
            )
            ...
            bci = self.get_gro_attribute_by_attributes(
                'chain_idx', {'globalIdx': bidx}
            )
            logger.debug(
                f' -> {aidx}-{bidx}: ac {ac} #{len(chainlists[ac])} aci {aci} bc {bc} #{len(chainlists[bc])} bci {bci}'
            )
            # one must be a head and the other a tail
            if aci == 0:    # a is a head
                if len(chainlists[bc]) - 1 != bci:
                    logger.warning(
                        f'Atom {bidx} has index {bci} in chain {bc} but that chain has {len(chainlists[bc])} elements, so it cannot be the tail of the chain. The tail appears to be atom {chainlists[bc][-1]}. So something is wrong, and I am not merging these chains.'
                    )
                ...
                if len(chainlists[ac]) - 1 != aci:
                    logger.warning(
                        f'Atom {aidx} has index {aci} in chain {ac} but that chain has {len(chainlists[ac])} elements, so it cannot be the tail of the chain. The tail appears to be atom {chainlists[ac][-1]}. So something is wrong, and I am not merging these chains.'
                    )
                    logger.debug(f'chain of {aidx} {chainlists[ac]}')
                    logger.debug(f'chain of {bidx} {chainlists[bc]}')
                    continue
                ...
                logger.debug(
                    f'Neither of {aidx} or {bidx} are heads of their respective chains.'
                )
                logger.debug(
                    f'This is most likely happening because of branching, which is permitted but'
                )
            ...
            for aidx in chainlists[c1]:
                self.set_gro_attribute_by_attributes(
                    'chain', c2, {'globalIdx': aidx}
                )
                self.set_gro_attribute_by_attributes(
                    'chain_idx', chainlists[c2].index(aidx), {'globalIdx': aidx}
                )
    ...
    def makes_cycle(self, aidx, bidx):
        ...
        for c in self.idx_lists['chain']:
            if (aidx == c[0]
                and bidx == c[-1]) or (aidx == c[-1] and bidx == c[0]):
                return True
        return False
    ...
    def cycle_collective(self, bdf: pd.DataFrame):
        ...
        def addlink(chainlist, i, j):
            ...
            if not (i_cidx == (-1, -1) or (j_cidx == (-1, -1))):
                ...
                old_head, old_tail = (i_cidx, j_cidx
                                     ) if i_cidx[1] == 0 else (j_cidx, i_cidx)
    ...
    def get_bystanders(self, atom_idx):
        ...
        bystander_atomidx = [
            [int], [int]
        ]    # elements correspond to atoms in atom_idx
        ...
        for x in [0, 1]:
            ...
            if atom_idx[
                1 -
                x] in bystander_atomidx[x]:    # if new bond has not yet formed
                bystander_atomidx[x].remove(atom_idx[1 - x])
            bystander_atomnames[x] = [
                self.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': y}
                ) for y in bystander_atomidx[x]
            ]
            bystander_resids[x] = [
                self.get_gro_attribute_by_attributes('resNum', {'globalIdx': y})
                for y in bystander_atomidx[x]
            ]
            bystander_resnames[x] = [
                self.get_gro_attribute_by_attributes(
                    'resName', {'globalIdx': y}
                ) for y in bystander_atomidx[x]
            ]
    ...
    def get_oneaways(self, atom_idx):
        ...
        chains = [
            self.get_gro_attribute_by_attributes('chain', {'globalIdx': x})
            for x in atom_idx
        ]
        chain_idx = [
            self.get_gro_attribute_by_attributes('chain_idx', {'globalIdx': x})
            for x in atom_idx
        ]
        ...
        if chains != [-1, -1]:
            ...
            for x in [0, 1]:
                ...
                for y in chainlists_idx[x]:
                    rnum, rname, aname = self.get_gro_attribute_by_attributes(
                        ['resNum', 'resName', 'atomName'], {'globalIdx': y}
                    )
            ...
            for i in [0, 1]:
                ...
                if oa_idx[i]:
                    ...
                    oneaway_atomnames[i] = chainlists_atomnames[i][
                        oa_chain_idx[i]]
    ...
    def grab_files(self):
        ...
        for ext in ['top', 'gro', 'grx']:
            ...
            logger.debug(
                f'{ext} grabbing {pfs.proj_abspath(filename)} into {pfs.cwd()}'
            )
    ...
    def grompp_and_mdrun(self, out, mdp, mylogger=logger.debug, **kwargs):
        ...
        self.copy_coords(
            TopoCoord(grofilename=f'{out}.gro', wrap_coords=wrap_coords)
        )
    ...
    def load_files(self, filenames: dict):
        ...
        for e, n in filenames.items():
            ...
            if ext == '.gro':
                ...
                self.read_gro_attributes(n)
            elif ext == '.mol2':
                self.read_mol2(n)
            else:
                logger.debug(
                    f'Warning: file {n} has unknown file extension.  Skipped.'
                )
    ...
    def vacuum_simulate(self, outname='simulated', **kwargs):
        ...
        logger.debug(
            f'nsteps {nsteps} begin_at {begin_at} ps {ps} sampling_duration_ps {sampling_duration_ps} ({sampling_duration_nsteps} steps) sample_interval {sample_interval}'
        )
    ...
    def equilibrate(self, deffnm='equilibrate', edict={}, gromacs_dict={}):
        ...
        self.grompp_and_mdrun(
            out=f'{deffnm}-{ens}', mdp=new_mdp, quiet=False, **gromacs_dict
        )
        ...
        for rep in range(repeat):
            ...
            self.grompp_and_mdrun(
                out=this_deffnm, mdp=new_mdp, quiet=False, **gromacs_dict
            )
            ...
            gmx_energy_trace(
                this_deffnm, ['Density'], report_averages=True, **gromacs_dict
            )
        return edr_list
    ...
    def get_resid_sets(self, atom_pair):
        ...
        if set(ci) == set(cj):
            logger.debug(
                f'resid sets overlap: {i} {ri} : {ci} and {j} {rj} : {cj}'
            )
            return []
        return [ci, cj]
