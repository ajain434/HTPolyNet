...
class Command:
    ...
    def run(self, override=(), ignore_codes=[], quiet=True):
        ...
        if process.returncode != 0 and not process.returncode in ignore_codes:
            ...
            if len(out) > 0:
                logger.error(
                    'stdout buffer follows\n' + '*' * self.linelen + '\n' + out +
                    '\n' + '*' * self.linelen
                )
            if len(err) > 0:
                logger.error(
                    'stderr buffer follows\n' + '*' * self.linelen + '\n' + err +
                    '\n' + '*' * self.linelen
                )
            raise subprocess.SubprocessError(
                f'Command "{self.c}" failed with returncode {process.returncode}'
            )
        else:
            if len(override) == 2:
                needle, msg = override
                if needle in out or needle in err:
                    logger.error(
                        f'Returncode: {process.returncode}, but another error was detected:'
                    )
                    logger.error(msg)
                    if len(out) > 0:
                        logger.error(
