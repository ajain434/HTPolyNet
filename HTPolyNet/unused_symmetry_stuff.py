...
def add_enumerated_angles(self, newbonds, ignores=[], quiet=True):
    ...
    for b in newbonds:
        ...
        for ai in [
            i for i in self.bondlist.partners_of(b[0])
            if (i != b[1] and not i in ignores)
        ]:
            aj = b[0]
            ak = b[1]
            ...
            if idx in ijk.index:
                ...
                if not quiet:
                    logger.warning(
                        f'Angle type {idx} ({ai}-{aj}-{ak}) not found.'
                    )
                angletype = 1
            ...
            self.D['angles'] = pd.concat(
                (self.D['angles'], pd.DataFrame(angledict)), ignore_index=True
            )
            newangles.append([i, j, k])
        ''' new angles due to other neighbors of b[1] '''
        for ak in [
            k for k in self.bondlist.partners_of(b[1])
            if (k != b[0] and not k in ignores)
        ]:
            ai = b[0]
            aj = b[1]
            it = at.iloc[ai - 1].type
...
def add_enumerated_dihedrals(self, newbonds, ignores=[], quiet=True):
    ...
    for b in newbonds:
        aj, ak = idxorder(b)
        for ai in [
            i for i in self.bondlist.partners_of(aj)
            if (i != ak and not i in ignores)
        ]:
            for al in [
                l for l in self.bondlist.partners_of(ak)
                if (l != aj and not l in ignores)
            ]:
                it = at.iloc[ai - 1].type
                jt = at.iloc[aj - 1].type
                kt = at.iloc[ak - 1].type
                ...
                if idx in ijkl.index:
                    dihedtype = ijkl.loc[idx,
                                         'func'].values[0]    # why values[0]
                else:
                    if not quiet:
                        logger.warning(
                            f'Dihedral type {idx} {ai}-{aj}-{ak}-{al} not found.'
                        )
                    dihedtype = 9
                h = _GromacsTopologyDirectiveHeaders_['dihedrals']
                data = [
                    i, j, k, l, dihedtype, pd.NA, pd.NA, pd.NA, pd.NA, pd.NA,
                    pd.NA
                ]
                assert len(h) == len(
                    data
                ), 'Error: not enough data for new  dihedral?'
                diheddict = {k: [v] for k, v in zip(h, data)}
                self.D['dihedrals'] = pd.concat(
                    (self.D['dihedrals'], pd.DataFrame(diheddict)),
                    ignore_index=True
                )
                ...
                self.D['pairs'] = pd.concat(
                    (self.D['pairs'], pd.DataFrame(pairdict)), ignore_index=True
                )
                newpairs.append((i, l))
        ''' new proper dihedrals for which the new bond is the i-j or j-i bond '''
        for ai, aj in zip(b, reversed(b)):
            for ak in [
                k for k in self.bondlist.partners_of(aj)
                if (k != ai and not k in ignores)
            ]:
                for al in [
                    l for l in self.bondlist.partners_of(ak)
                    if (l != aj and not l in ignores)
                    ...
                    i, j, k, l = idxorder((ai, aj, ak, al))
                    repeat_check(
                        (i, j, k, l), msg=f'i-j neighbor of j-k {j}-{k}'
                    )
                    if idx in ijkl.index:
                    ...
                    pairdict = {k: [v] for k, v in zip(h, data)}
                    self.D['pairs'] = pd.concat(
                        (self.D['pairs'], pd.DataFrame(pairdict)),
                        ignore_index=True
                    )
                    newpairs.append((i, l))
        ...
        for ak, al in zip(b, reversed(b)):
            for aj in [
                j for j in self.bondlist.partners_of(ak)
                if (j != al and not j in ignores)
            ]:
                for ai in [
                    i for i in self.bondlist.partners_of(aj)
                    if (i != ak and not i in ignores)
                    ...
                    i, j, k, l = idxorder((ai, aj, ak, al))
                    repeat_check(
                        (i, j, k, l), msg=f'k-l neighbor of j-k {j}-{k}'
                    )
                    if idx in ijkl.index:
                        dihedtype = ijkl.loc[idx, 'func'].values[0]
                    else:
