...
class Runtime:
    ...
    def generate_molecules(
        self, force_parameterization=False, force_checkin=False
    ):
        ...
        logger.info(
            f'{len(self.cfg.molecules)} molecule{ess} detected in {self.cfgfile}'
        )
        replns = [
            f'{msg:>30s}: {count:<5d}'
            for msg, count in self.cfg.molecule_report.items()
        ]
        for ln in replns:
            logger.info(ln)
        ...
        if len(new_molecules) > 0:
            ...
            logger.info(
                f'{len(new_molecules)} molecule{ess} implied by chaining'
            )
        ...
        for M in self.molecules:
            self.molecules[M].is_reactant = is_reactant(
                M, self.cfg.reactions, stage=reaction_stage.cure
            )

        resolve_type_discrepancies = GAFF_dict.get(
            'resolve_type_discrepancies', []
        )
        if not resolve_type_discrepancies:
            resolve_type_discrepancies = self.cfg.parameters.get(
                'resolve_type_discrepancies', []
            )
        if resolve_type_discrepancies:
            for resolve_directive in resolve_type_discrepancies:
                logger.info(
                    f'Resolving type discrepancies for directive {resolve_directive}'
                )
                self._type_consistency_check(
                    typename=resolve_directive['typename'],
                    funcidx=resolve_directive.get('funcidx', 4),
                    selection_rule=resolve_directive['rule']
                )
        ...
        if self.cfg.initial_composition:
            cmp_msg = ", ".join(
                [
                    (x["molecule"] + " " + str(x["count"]))
                    for x in self.cfg.initial_composition if x["count"] > 0
                ]
            )
    ...
    def do_densification(self, deffnm='densified'):
        ...
        assert all(
            [os.path.exists(x) for x in infiles]
        ), f'One or more of {infiles} not found'
        self._do_equilibration_series(
            equilibration, deffnm=f'{deffnm}', plot_pfx='densification'
        )
        logger.info(
            f'Densified coordinates in {pfs.cwd()}/{os.path.basename(TC.files["gro"])}'
        )
        return {
            c: os.path.basename(x)
            for c, x in TC.files.items() if c != 'mol2'
        }
    ...
    def do_precure(self):
        ...
        return {
            c: os.path.basename(x)
            for c, x in self.TopoCoord.files.items() if c != 'mol2'
        }
    ...
    def do_cure(self):
        ...
        if os.path.exists('systems/cure_state.yaml'):
            ...
            my_logger(
                f'Connect-Update-Relax-Equilibrate (CURE) resumes', logger.info
            )
            my_logger(
                f'at iteration {cc.state.iter} and {cc.state.cum_nxlinkbonds} bonds',
                logger.info
            )
        else:
            cc.setup(
                max_nxlinkbonds=self.cfg.maxconv,
                desired_nxlinkbonds=int(
                    self.cfg.maxconv * cc.dicts['controls']['desired_conversion']
                ),
                max_search_radius=float(min(TC.Coordinates.box.diagonal() / 2))
            )
            cc.state.iter = 1
            my_logger(
                'Connect-Update-Relax-Equilibrate (CURE) begins', logger.info
            )
    ...
    def _generate_molecule(self, M: Molecule, **kwargs):
        ...
        :type M: Molecule
        """
        if M.origin != 'unparameterized' and M.origin != 'symmetry_product':
            return
        mname = M.name
        checkin = pfs.checkin
        force_parameterization = kwargs.get('force_parameterization', False)
        ...
        if force_parameterization or not M.previously_parameterized():
            logger.debug(
                f'Parameterization of {mname} requested -- can we generate {mname}?'
            )
            generatable = (not M.generator) or (
                all(
                    [
                        m in self.molecules
                        for m in M.generator.reactants.values()
                    ]
                )
            )
            if generatable:
                logger.debug(f'Generating {mname}')
                M.generate(
                    available_molecules=self.molecules, **self.cfg.parameters
                )
                for ex in ['mol2', 'top', 'itp', 'gro', 'grx']:
                    checkin(
                        f'molecules/parameterized/{mname}.{ex}',
                        overwrite=force_checkin
                    )
                M.set_origin('newly parameterized')
            else:
                logger.debug(f'Error: could not generate {mname}')
                logger.debug(
                    f'not ({mname}.generator) = {bool(not M.generator)}'
                )
                if M.generator:
                    logger.debug(
                        f'reactants {list(M.generator.reactants.values())}'
                    )
                return
        else:
    ...
    def _type_consistency_check(
        self, typename='dihedraltypes', funcidx=4, selection_rule='stiffest'
    ):
        ...
        """
        logger.debug(
            f'Consistency check of {typename} func {funcidx} on all {len(self.molecules)} molecules requested'
        )
        ...
        for i in range(len(mnames)):
            ...
            for j in range(i + 1, len(mnames)):
                ...
                this_duptypes = mol1topo.report_duplicate_types(
                    mol2topo, typename=typename, funcidx=funcidx
                )
        ...
        for t in types_duplicated:
            ...
            for i in range(len(mnames)):
                ...
                this_type = moltopo.report_type(
                    t, typename='dihedraltypes', funcidx=4
                )
            ...
            selected_type = select_topology_type_option(
                options[t], typename, rule=selection_rule
            )
            logger.debug(
                f'Under selection rule "{selection_rule}", preferred type is {selected_type}'
            )
            for i in range(len(mnames)):
    ...
    def _initialize_topology(self, inpfnm='init'):
        ...
        for item in self.cfg.initial_composition:
            ...
            logger.debug(
                f'Merging {N} copies of {M.name}\'s topology into global topology'
            )
            t.adjust_charges(
                atoms=t.D['atoms']['nr'].to_list(),
                desired_charge=0.0,
                overcharge_threshhold=0.1,
                msg=''
            )
            t.rep_ex(N)
            TC.Topology.merge(t)
        ...
        for othermol, M in self.molecules.items():
            if not othermol in already_merged:
                logger.debug(
                    f'Merging types from {othermol}\'s topology into global topology'
                )
    ...
    def _initialize_coordinates(self, inpfnm='init'):
        ...
        if not densification_dict:
            densification_dict['aspect_ratio'] = self.cfg.parameters.get(
                'aspect_ratio', np.array([1., 1., 1.])
            )
            if 'initial_density' in self.cfg.parameters:
                densification_dict['initial_density'] = self.cfg.parameters[
                    'initial_density']
        ...
        assert any(
            dspec
        ), 'Neither "initial_boxsize" nor "initial_density" are specfied'
        assert not all(
            dspec
        ), 'Cannot specify both "initial_boxsize" and "initial_density"'
        if 'initial_boxsize' in densification_dict:
            ...
            assert ar[
                0
            ] == 1., f'Error: parameter aspect_ratio must be a 3-element-list with first element 1'
            ar_yx = ar[1] * ar[2]
            L0_m = (V0_m3 / ar_yx)**(1. / 3.)
            L0_nm = L0_m * 1.e9
            boxsize = np.array([L0_nm, L0_nm, L0_nm]) * ar
            logger.info(
                f'Initial density: {densification_dict["initial_density"]} kg/m^3'
            )
            logger.info(f'Total mass: {mass_kg:.3e} kg')
            logger.info(f'Box aspect ratio: {" x ".join([str(x) for x in ar])}')
        logger.info(
            f'Initial box side lengths: {boxsize[0]:.3f} nm x {boxsize[1]:.3f} nm x {boxsize[2]:.3f} nm'
        )
        ...
        for cc in clist:
            ...
            if len(M.conformers) > 0:
                ...
                if tc < nconf:
                    c_togromacs.update(
                        {u: 1
                         for u in random.sample(M.conformers, tc)}
                    )
                else:
                    q, r = divmod(tc, nconf)
                ...
                for gro in M.conformers:
                    pfs.checkout(
                        f'molecules/parameterized/{gro}.gro',
                ...
                if leftovers > 0:
                    ...
                    leftovers -= 1
                pfs.checkout(
                    f'molecules/parameterized/{M.name}.gro',
                    altpath=[pfs.subpath('molecules')]
                )
        ...
        msg = insert_molecules(
            ...
            boxsize,
            inpfnm,
            inputs_dir='../../molecules/parameterized',
            **self.cfg.parameters
        )
        TC.read_gro(f'{inpfnm}.gro')
        TC.atom_count()
        TC.set_grx_attributes()
        TC.inherit_grx_attributes_from_molecules(
            self.cfg.molecules, self.cfg.initial_composition
        )
        ...
        if ens == 'npt' and plot_pfx != '':
            trace(
                ...
                outfile=os.path.join(
                    pfs.proj(), f'plots/{plot_pfx}-density.png'
                )
            )

    def _do_equilibration_series(
        self, eq_stgs=[], deffnm='equilibrate', plot_pfx=''
    ):
    ...
    def _do_pap(self, pfx='precure'):
        ...
        if not any([preequil, anneal, postequil]):
            return
        if not _nonempty_directives(
            [x for x in [preequil, anneal, postequil] if x]
        ):
            return
        my_logger(f'{pfx.capitalize()} in {pfs.cwd()}', logger.info)
        if preequil:
            self._do_equilibration(
                preequil,
                deffnm='preequilibration',
                plot_pfx=f'{pfx}-preequilibration'
            )
        if anneal and _nonempty_directives([anneal]):
            self._do_anneal(anneal, deffnm='annealed')
            trace(
                'Temperature', ['annealed'],
                outfile=os.path.join(pfs.proj(), f'plots/{pfx}-anneal-T.png')
            )
        if postequil:
            self._do_equilibration(
                postequil,
                deffnm='postequilibration',
                plot_pfx=f'{pfx}-postequilibration'
            )
    ...
    def _do_anneal(self, anneal_dict={}, deffnm='anneal'):
        ...
        if not any(durations):
            durations = [
                '{:.2f}'.format(r.get('nsteps', ) * timestep)
                for r in cycle_segments
            ]
            if not any(durations):
        ...
        logger.info(
            f'Annealing: {len(durations)} points for {ncycles} cycles over {total_duration} ps'
        )
        mod_dict = {
        ...
        msg = TC.grompp_and_mdrun(
            out=deffnm, mdp=mdp_pfx, quiet=False, **gromacs_dict
        )
