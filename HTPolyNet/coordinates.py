...
GRX_ATTRIBUTES = [
    'z', 'nreactions', 'reactantName', 'sea_idx', 'cycle', 'cycle_idx', 'chain',
    'chain_idx', 'molecule', 'molecule_name'
]
...
class Coordinates:
    ...
    """
    gro_attributes = [
        'resNum', 'resName', 'atomName', 'globalIdx', 'posX', 'posY', 'posZ',
        'velX', 'velY', 'velZ'
    ]
    ...
    mol2_atom_attributes = [
        'globalIdx', 'atomName', 'posX', 'posY', 'posZ', 'type', 'resNum',
        'resName', 'charge'
    ]
    ...
    mol2_bond_types = {
        k: v
        for k, v in zip(mol2_bond_attributes, [int, int, int, str])
    }
    ...
    def read_gro(cls, filename, wrap_coords=True):
        ...
        if filename != '':
            with open(filename, 'r') as f:
                ...
                assert inst.N == len(
                    series['globalIdx']
                ), f'Atom count mismatch inside {filename}'
                ...
                if len(boxdata) == 9:
                    inst.box[0][1], inst.box[0][2], inst.box[1][0], inst.box[1][
                        2], inst.box[2][0], inst.box[2][1] = boxdata[3:]
        inst.empty = False
    ...
    def read_mol2(cls, filename):
        ...
        with open(filename, 'r') as f:
            ...
            inst.A = pd.read_csv(
                sections['atom'],
                sep='\s+',
                names=Coordinates.mol2_atom_attributes
            )
            inst.A[['posX', 'posY', 'posZ']] *= [0.1, 0.1, 0.1]
            inst.N = inst.A.shape[0]
            inst.mol2_bonds = pd.read_csv(
                sections['bond'],
                sep='\s+',
                names=Coordinates.mol2_bond_attributes,
                dtype=Coordinates.mol2_bond_types
            )
    ...
    def copy_coords(self, other):
        ...
        """
        assert self.A.shape[0] == other.A.shape[
            0
        ], f'Cannot copy -- atom count mismatch {self.A.shape[0]} vs {other.A.shape[0]}'
    ...
    def linkcell_initialize(
        ...
        if populate:
            ...
            if os.path.exists(lc_file) and not force_repopulate:
                ...
                logger.debug(
                    f'Read linkcell_idx from {lc_file} {("linkcell_idx" in self.A)} {results}'
                )
                self.linkcell.make_memberlists(self.A)
            else:
                self.set_atomset_attribute(
                    'linkcell_idx', -1 * np.ones(self.A.shape[0]).astype(int)
                )
                sc = self.subcoords(
                    self.A[(self.A['cycle_idx'] > 0) | (self.A['z'] > 0)].copy()
                )
    ...
    def rij(self, i, j, pbc=[1, 1, 1]):
        ...
        if np.any(pbc) and not np.any(self.box):
            logger.warning(
                'Interatomic distance calculation using PBC with no boxsize set.'
            )
        ri = self.get_R(i)
        rj = self.get_R(j)
    ...
    def write_atomset_attributes(self, attributes, filename, formatters=[]):
        ...
        for a in attributes:
            if not a in self.A.columns:
                raise Exception(
                    f'There is no column "{a}" in this atoms dataframe'
                )
        with open(filename, 'w') as f:
            if len(formatters) > 0:
                f.write(
                    self.A[['globalIdx'] + attributes].
                    to_string(header=True, index=False, formatters=formatters) +
                    '\n'
                )
            else:
                f.write(
                    self.A[['globalIdx'] +
                           attributes].to_string(header=True, index=False) + '\n'
                )
    ...
    def read_atomset_attributes(self, filename, attributes=[]):
        ...
        if len(attributes) == 0:
            ...
            df = pd.read_csv(
                filename, sep='\s+', names=['globalIdx'] + attributes, header=0
            )
    ...
    def minimum_distance(self, other, self_excludes=[], other_excludes=[]):
        ...
        :rtype: float
        """
        sp = self.A[~self.A['globalIdx'].isin(self_excludes)][[
            'posX', 'posY', 'posZ'
        ]]
        op = other.A[~other.A['globalIdx'].isin(other_excludes)][[
            'posX', 'posY', 'posZ'
        ]]
        minD = 1.e9
        for i, srow in sp.iterrows():
    ...
    def minmax(self):
        ...
        """
        sp = self.A[['posX', 'posY', 'posZ']]
        return np.array([sp.posX.min(),
                         sp.posY.min(),
                         sp.posZ.min()]), np.array(
                             [sp.posX.max(),
                              sp.posY.max(),
                              sp.posZ.max()]
                         )
    ...
    def get_R(self, idx):
        ...
        return get_row_attribute(
            df, ['posX', 'posY', 'posZ'], {'globalIdx': idx}
        )
    ...
    def sacH(self, ai, aj, T, rename=False):
        ...
        i_Hpartners = {
            ...
            for k, v in zip(
                i_partners, [
                    self.A[self.A['globalIdx'] == i]['atomName'].values[0]
                    for i in i_partners
                ]
            ) if v.startswith('H')
        }
        ...
        assert len(
            i_Hpartners
        ) > 0, f'Error: atom {ai} does not have a deletable H atom!'
        assert len(
            j_Hpartners
        ) > 0, f'Error: atom {aj} does not have a deletable H atom!'
        minHH = (1.e9, -1, -1)
    ...
    def delete_atoms(self, idx=[], reindex=True):
        ...
        if not self.mol2_bonds.empty:
            ...
            self.mol2_bonds = d.take(list(indexes_to_keep)
    ...
    def write_gro(self, filename, grotitle=''):
        ...
        with open(filename, 'w') as f:
            ...
            atomformatters = [
                lambda x: f'{x:>5d}', lambda x: f'{x:<5s}', lambda x: f'{x:>5s}',
                lambda x: f'{x%10000:5d}'
            ] + [lambda x: f'{x:8.3f}'] * 3 + [lambda x: f'{x:8.4f}'] * 3
            ...
            for i, r in self.A.iterrows():
                if has_vel:
                    f.write(
                        ''.join(
                            [
                                atomformatters[i](v) for i, v in
                                enumerate(list(r[self.gro_attributes]))
                            ]
                        ) + '\n'
                    )
                else:
                    f.write(
                        ''.join(
                            [
                                atomformatters[i](v) for i, v in
                                enumerate(list(r[self.gro_attributes[:-3]]))
                            ]
                        ) + '\n'
                    )
            if not np.any(self.box):
                logger.debug(
                    'Writing Gromacs coordinates file but boxsize is not set.'
                )
            f.write(
                f'{self.box[0][0]:10.5f}{self.box[1][1]:10.5f}{self.box[2][2]:10.5f}'
            )
    ...
    def write_mol2(
        ...
        if bondsDF.empty and self.mol2_bonds.empty:
            ...
            logger.info(
                'Coordinates.write_mol2 provided with both a bondsDF parameter and a mol2_bonds attribute'
            )
        ...
        if filename != '':
            atomformatters = [
                lambda x: f'{x:>7d}', lambda x: f'{x:<8s}',
                lambda x: f'{x:>9.4f}', lambda x: f'{x:>9.4f}',
                lambda x: f'{x:>9.4f}', lambda x: f'{x:<5s}',
                lambda x: f'{x:>3d}', lambda x: f' {x:<7s}',
                lambda x: f'{x:>9.4f}'
            ]
            bondformatters = [
                lambda x: f'{x:>6d}', lambda x: f'{x:>5d}', lambda x: f'{x:>5d}',
                lambda x: f'{str(x):>4s}'
            ]
            substructureformatters = [
                lambda x: f'{x:>6d}', lambda x: f'{x:<7s}', lambda x: f'{x:>6d}',
                lambda x: f'{x:<7s}'
            ]
            with open(filename, 'w') as f:
                ...
                f.write(
                    '{:>6d}{:>6d}{:>3d}{:>3d}{:>3d}\n'.format(
                        N, nBonds, nSubs, nFeatures, nSets
                    )
                )
                ...
                f.write(
                    acopy.to_string(
                        columns=self.mol2_atom_attributes,
                        header=False,
                        index=False,
                ...
                if not bondsDF.empty:
                    ...
                    f.write(
                        bdf.to_string(
                            columns=self.mol2_bond_attributes,
                            header=False,
                            index=False,
                    ...
                    logger.debug(
                        f'write_mol2 ({filename}): Mol2 bonds from mol2_bonds attribute'
                    )
                    f.write(
                        self.mol2_bonds.to_string(
                            columns=self.mol2_bond_attributes,
                            header=False,
                            index=False,
                            formatters=bondformatters
                ...
                f.write(
                    rdf.to_string(
                        header=False,
                        index=False,
                        formatters=substructureformatters
                    )
                )
                f.write('\n')
