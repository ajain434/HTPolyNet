...
def insert_molecules(composition, boxSize, outName, inputs_dir='.', **kwargs):
    ...
    for name, num in composition.items():    # composition determines order
        ...
        if os.path.isfile(f'{outName}.gro'):
            logger.debug(
                f'gmx insert-molecules inserts into existing {outName}.gro'
            )
            ''' final gro file exists; we must insert into it '''
            c = Command(
                f'{sw.gmx} {sw.gmx_options} insert-molecules',
                f=f'{outName}.gro',
                ci=ci,
                nmol=num,
                o=outName,
...
def grompp_and_mdrun(
    gro='', top='', out='', mdp='', boxSize=[], single_molecule=False, **kwargs
):
    ...
    for option in [
        'rdd', 'dds', 'dlb', 'npme', 'nt', 'ntpmi', 'ntomp', 'ntomp_pme', 'nb',
        'tunepme', 'pme', 'pmefft', 'bonded', 'update'
    ]:
    ...
    if gro == '' or top == '' or out == '' or mdp == '':
        raise Exception(
            'grompp_and_mdrun requires gro, top, out, and mdp filename prefixes.'
        )
    ...
    if len(boxSize) > 0:
        ...
        c = Command(
            f'{sw.gmx} {sw.gmx_options} editconf',
            f=f'{gro}.gro',
            o=gro,
            box=' '.join([f'{x:.8f}' for x in boxSize])
        )
    ...
    c = Command(
        f'{sw.gmx} {sw.gmx_options} grompp',
        f=f'{mdp}.mdp',
        c=f'{gro}.gro',
        p=f'{top}.top',
        o=f'{out}.tpr',
...
def get_energy_menu(edr, **kwargs):
    ...
    with open('_menugetter_', 'w') as f:
        f.write('\n')
    c = Command(
        f'{sw.gmx} {sw.gmx_options} energy -f {edr}.edr -o {edr}-out.xvg -xvg none < _menugetter_ > _menu_ 2>&1'
    )
...
def gmx_energy_trace(
    edr, names=[], report_averages=False, keep_files=False, **kwargs
):
    ...
    c = Command(
        f'{sw.gmx} {sw.gmx_options} energy -f {edr}.edr -o {edr}-out.xvg -xvg none < {edr}-gmx.in'
    )
    c.run()
    ...
    if report_averages:
        for i in names:
            ...
            data[f'Rolling-10-average-{i}'] = data[i].rolling(
                window=ndata // 10
            ).mean()
            for ln in data.iloc[-1][[
                i, f'Running-average-{i}', f'Rolling-10-average-{i}'
            ]].to_string(float_format='{:.2f}'.format).split('\n'):
                logger.info(f'{ln}')
    if not keep_files:
...
def gromacs_distance(
    ...
    with open(f'{pfx}.in', 'w') as f:
        f.write('0\n\n')
    c = Command(
        f'{sw.gmx} {sw.gmx_options} distance -f {gro} -n {pfx}.ndx -oall {pfx}.xvg -xvg none < {pfx}.in'
    )
    c.run()
...
def gro_from_trr(pfx, nzero=2, b=0, outpfx=''):
    ...
    with open('tmp.in', 'w') as f:
        f.write('0\n')
    c = Command(
        f'{sw.gmx} {sw.gmx_options} trjconv -f {pfx}.trr -s {pfx}.tpr -o {outpfx}.gro -sep -nzero {nzero} -b {b} < tmp.in'
    )
    out, err = c.run()
    os.remove('tmp.in')
    out += err
