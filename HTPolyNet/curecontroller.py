...
class CureController:
    ...
    def __init__(self, curedict={}):
        ...
        for k, v in self.curedict_defaults.items():
            self.dicts[k] = curedict.get(
                k, v
            )    # loads entire default if dict is just missing
            if type(
                v
            ) == dict:    # assign subitem defaults if their keys are missing
    ...
    def setup(
        self, max_nxlinkbonds=0, desired_nxlinkbonds=0, max_search_radius=0.0
    ):
    ...
    def do_iter(
        ...
        logger.info(
            f'Iteration {self.state.iter} current conversion {self._curr_conversion():.3f} or {self.state.cum_nxlinkbonds} bonds'
        )
        return {
    ...
    def is_cured(self):
        ...
        logger.debug(
            f'cumxlinks {st.cum_nxlinkbonds} maxxlinks {st.max_nxlinkbonds}: {(st.cum_nxlinkbonds>=st.max_nxlinkbonds)}'
        )
        finished = self.search_failed or (
            st.cum_nxlinkbonds >= st.desired_nxlinkbonds
        )
        if finished:
    ...
    def _curr_conversion(self):
        ...
        :rtype: float
        """
        if not self.state.max_nxlinkbonds:
            return 0
    ...
    def _do_bondsearch(
        self, TC: TopoCoord, RL: ReactionList, MD: MoleculeDict, reentry=False
    ):
        ...
        """
        if self.state.step != cure_step.cure_bondsearch:
            return
        opfx = self._pfx()
        d = self.dicts['controls']
        self.state.current_radius = d[
            'search_radius'] + self.state.current_radidx * d['radial_increment']
        logger.info(
            f'Bond search using radius {self.state.current_radius} nm initiated'
        )
        curr_conversion = self._curr_conversion()
        apply_probabilities = curr_conversion < d['late_threshold']
        bond_limit = int(
            d['max_conversion_per_iteration'] * self.state.max_nxlinkbonds
        )
        bond_target = int(
            (d['desired_conversion'] - curr_conversion) *
            self.state.max_nxlinkbonds
        )
        ...
        while nbonds == 0 and self.state.current_radidx < self.state.max_radidx:
            ...
            if nbonds == 0:
                ...
                logger.info(
                    f'Radius increased to {self.state.current_radius} nm'
                )
        if nbonds > 0:
            ess = '' if nbonds == 1 else 's'
            logger.info(
                f'Iteration {self.state.iter} will generate {nbdf.shape[0]} new bond{ess}'
            )
            ...
            self._register_bonds(
                nbdf, pairs, f'{opfx}-bonds.csv', bonds_are='identified'
            )
    ...
    def _do_preupdate_dragging(self, TC: TopoCoord, gromacs_dict={}):
        ...
        :type gromacs_dict: dict, optional
        """
        if self.state.step != cure_step.cure_drag:
            return
        nbdf = self.bonds_df
        assert nbdf.shape[0] > 0
        d = self.dicts['drag']
        nogos = [
            not self.dragging_enabled, nbdf['initial_distance'].max()
            < d['trigger_distance']
        ]
        ...
        if any(nogos):
            ...
            self._distance_attenuation(
                TC, mode='drag', gromacs_dict=gromacs_dict
            )
    ...
    def _do_relax(self, TC: TopoCoord, gromacs_dict={}):
        ...
        :type gromacs_dict: dict, optional
        """
        if self.state.step != cure_step.cure_relax and self.state.step != cure_step.cap_relax:
            return
        nbdf = self.bonds_df
    ...
    def _do_topology_update(self, TC: TopoCoord, MD: MoleculeDict):
        ...
        :type MD: MoleculeDict
        """
        if self.state.step != cure_step.cure_update and self.state.step != cure_step.cap_update:
            return
        opfx = self._pfx()
        logger.debug(f'Topology update')
        bonds_df, pairs_df = TC.update_topology_and_coordinates(
            self.bonds_df,
            template_dict=MD,
            write_mapper_to=f'{opfx}-idx-mapper.csv'
        )
    ...
    def _do_equilibrate(self, TC: TopoCoord, gromacs_dict={}):
        ...
        :type gromacs_dict: dict, optional
        """
        if self.state.step != cure_step.cure_equilibrate and self.state.step != cure_step.cap_equilibrate:
            return
        d = self.dicts['equilibrate']
        opfx = self._pfx()
        edr_list = TC.equilibrate(
            deffnm=f'{opfx}', edict=d, gromacs_dict=gromacs_dict
    ...
    def _do_cap_bondsearch(
        self, TC: TopoCoord, RL: ReactionList, MD: MoleculeDict
    ):
        ...
        :type MD: MoleculeDict
        """
        if self.state.step != cure_step.cap_bondsearch:
            return
        opfx = self._pfx()
    ...
    def _read_bonds_df(self, bonds_file_override=''):
        ...
        """
        infile = self.dicts['output'][
            'bonds_file'] if not bonds_file_override else bonds_file_override
    ...
    def _distance_attenuation(self, TC: TopoCoord, mode='drag', gromacs_dict={}):
        ...
        maxL, minL, meanL = nbdf['current_lengths'].max(
        ), nbdf['current_lengths'].min(), nbdf['current_lengths'].mean()
        ...
        if mode == 'drag':
            ...
            pmaxL, pminL, pmeanL = pdf['current_lengths'].max(
            ), pdf['current_lengths'].min(), pdf['current_lengths'].mean()
            logger.debug(
        ...
        for stg_dict in d['equilibration']:
            ...
            if ensemble != 'min':
                mdp_mods_dict.update(
                    {
                        'gen-temp': stg_dict['temperature'],
                        'ref_t': stg_dict['temperature'],
                        'gen-vel': 'yes',
                        'nsteps': stg_dict['nsteps']
                    }
                )
            if ensemble == 'npt':
        ...
        for i in range(this_firststage, this_nstages):
            ...
            logger.debug(
                f'Distances avg/min/max: {meanL:.3f}/{minL:.3f}/{maxL:.3f}'
            )
            if mode == 'drag':
        ...
        self._register_bonds(
            nbdf,
            pdf,
            f'{opfx}-{mode}-bonds.csv',
            bonds_are=('relaxed' if mode == 'relax' else 'dragged')
        )
    ...
    def _searchbonds(
        ...
        raset = adf[
            adf['z'] > 0
        ]    # this view will be used for downselecting to potential A-B partners
        bdf = pd.DataFrame()
        ...
        for R in Rlist:
            ...
            for bond in R.bonds:
                ...
                aresname = MD[areactantname_template].get_resname(
                    aresid_template
                )
                az = A['z']
                bname = B['atom']
                ...
                bresname = MD[breactantname_template].get_resname(
                    bresid_template
                )
                bz = B['z']
                ...
                if stage == reaction_stage.cure:
                    ...
                    if idf.shape[0] > 0:
                        ...
                        logger.debug(
                            f'Decomposed dataframe lengths: {", ".join([str(x.shape[0]) for x in idf_split])}'
                        )
                        results = p.map(
                            partial(
                                gromacs_distance, gro=gro, new_column_name='r'
                            ), packets
                        )
                        p.close()
                        p.join()
        ...
        if stage == reaction_stage.cure and bdf.shape[0] > 0:
            ...
            logger.debug(
                f'{bdf[bdf["allowed"]==False].shape[0]} out of {bdf.shape[0]} bonds disallowed due to repeated atom indexes or residue indexes'
            )
            ...
            logger.debug(
                f'{bdf[bdf["remove-to-uncyclize"]==True].shape[0]} out of {bdf.shape[0]} bonds removed to break nascent cycles'
            )
            bdf = bdf[bdf['remove-to-uncyclize'] == False].copy().reset_index(
                drop=True
            )
            ...
            logger.debug(
                f'{bdf[bdf["lucky"]==True].shape[0]} bonds survive probability application'
            )
