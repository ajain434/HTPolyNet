    ...
    def parse(self, **kwargs):
        ...
        for d in self.basedict.keys():
            if not d in self.default_directives:
                logging.debug(
                    f'Ignoring unknown directive "{d}" in {self.cfgFile}'
                )
        ...
        mol_reac_detected = extract_molecule_reactions(
            self.reactions, plot=plot_reaction_network
        )
        ...
        for mname, gen in mol_reac_detected:
            self.molecules[mname] = Molecule.New(
                mname, gen, self.constituents.get(mname, {})
            )
        ...
        self.molecule_report['implied by stereochemistry'
                            ] += generate_stereo_reactions(
                                self.reactions, self.molecules
                            )
        self.molecule_report['implied by symmetry'
        ...
        for molecule, mrec in self.constituents.items():
            self.initial_composition.append(
                {
                    'molecule': molecule,
        ...
        for molec in self.initial_composition:
            ...
            if not mname in self.molecules:
                self.molecules[mname] = Molecule.New(
                    mname, None, self.constituents.get(mname, {})
                )
