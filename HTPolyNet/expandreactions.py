...
def chain_expand_reactions(molecules: MoleculeDict):
    ...
    for mname, M in molecules.items():
        if len(M.sequence) == 1 and len(
            M.TopoCoord.idx_lists['chain']
        ) > 0 and M.generator == None and M.parentname == M.name:
            monomers.append(M)
    ...
    for mon in monomers:
        ...
        for c in mon.TopoCoord.idx_lists['chain']:
            cnms.append(
                [
                    mon.TopoCoord.get_gro_attribute_by_attributes(
                        'atomName', {'globalIdx': x}
                    ) for x in c
                ]
            )
        logger.debug(
            f'Monomer {mon.name} has {len(mon.TopoCoord.idx_lists["chain"])} 2-chains: {mon.TopoCoord.idx_lists["chain"]} {cnms}'
        )
    ...
    for dim in dimer_lefts:
        ...
        for cl in dim.TopoCoord.idx_lists['chain']:
            nl = [
                dim.TopoCoord.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': x}
                ) for x in cl
            ]
            logger.debug(f'  -> {nl}')
    ...
    for m, d in MD:
        for mb in m.TopoCoord.idx_lists['chain']:
            h_idx = mb[0]
            h_name = m.TopoCoord.get_gro_attribute_by_attributes(
                'atomName', {'globalIdx': h_idx}
            )
            ...
            for DC in D4:
                t_idx = DC[-1]
                t_name = d.TopoCoord.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': t_idx}
                )
                ...
                R.product = new_mname
                newP = Molecule.New(R.product,
                                    R).set_sequence_from_moldict(molecules)
                newP.set_origin('unparameterized')
                extra_molecules[R.product] = newP
                logger.debug(
                    f'monomer atom {m.name}_{h_name} will attack dimer atom {d.name}[{d.sequence[0]}1_{t_name}] -> {new_mname}:'
                )
    ...
    for m, d in MD:
        for mb in m.TopoCoord.idx_lists['chain']:
            assert len(
                mb
            ) == 2, f'monomer {m.name} has a chain that is not length-2 -- this is IMPOSSIBLE'
            t_idx = mb[-1]
            t_name = m.TopoCoord.get_gro_attribute_by_attributes(
                'atomName', {'globalIdx': t_idx}
            )
            D4 = []
            for dc in d.TopoCoord.idx_lists['chain']:
            ...
            for DC in D4:
                h_idx = DC[0]
                h_name = d.TopoCoord.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': h_idx}
                )
                ...
                extra_molecules[R.product] = newP
                logger.debug(
                    f'dimer atom {d.name}[{d.sequence[1]}2_{h_name}] will attack monomer atom {m.name}_{t_name}-> {new_mname}:'
                )
                for ln in str(R).split('\n'):
                    logger.debug(ln)
                extra_reactions.append(R)
    ...
    for dr, dl in DD:
        ...
        for cr, cl in product(
            dr.TopoCoord.idx_lists['chain'], dl.TopoCoord.idx_lists['chain']
        ):
            if len(cr) == 4 and len(cl) == 4:
                h_idx = cr[0]
                h_name = dr.TopoCoord.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': h_idx}
                )
                t_idx = cl[-1]
                t_name = dl.TopoCoord.get_gro_attribute_by_attributes(
                    'atomName', {'globalIdx': t_idx}
                )
                ...
                extra_molecules[R.product] = newP
                logger.debug(
                    f'dimer atom {dr.name}-{dr.sequence[1]}2_{h_name} will attack dimer atom {dl.name}-{dl.sequence[0]}1_{t_name} -> {new_mname}:'
                )
                for ln in str(R).split('\n'):
                    logger.debug(ln)
                extra_reactions.append(R)
