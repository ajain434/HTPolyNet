...
class Linkcell:
    ...
    def create(self, cutoff, box, origin=np.array([0., 0., 0.])):
        ...
        self.cellndx = np.array(
            list(product(*[np.arange(x) for x in self.ncells]))
        )
        # 3-d array of lower left corner as a 3-space point, indexed by i,j,k
        for t in self.cellndx:
            i, j, k = t
            self.cells[i, j,
                       k] = self.celldim * np.array([i, j, k]) + self.origin
        # set up neighbor lists using linear indices
        self.make_neighborlists()
        logger.debug(
            f'Linkcell structure: {len(self.cellndx)} cells ({self.ncells}) dim {self.celldim}'
        )
    ...
    def cellndx_of_point(self, R):
        ...
        lowdim = (C < np.zeros(3).astype(int)).astype(
            int
        )    # will never happen if R is wrapped
        hidim = (C >= self.ncells).astype(int)    # could happen if exactly there
        if (any(lowdim) or any(hidim)):
            logger.warning(
                f'Warning: point {R} maps to out-of-bounds-cell {C} ({self.ncells})'
            )
    ...
    def populate(self, Coordinates, ncpu=1):
        ...
        logger.debug(
            f'Linkcell: assigning cell indices to {N} atoms in {self.box}...'
        )
        Coordinates.set_atomset_attribute(
            'linkcell_idx', -1 * np.ones(N).astype(int)
        )
        ...
        for i in idx_list:
            lc_idx = Coordinates.get_atom_attribute(
            ...
            try:
                ...
                logger.debug(
                    f'Linear linkcell index {lc_idx} of atom {i} is out of range.\ncellndx.shape[0] is {self.cellndx.shape[0]}\nThis is a bug.'
                )
                raise Exception
        ...
        result = p.map(
            partial(self._return_list_lens, mlists=self.memberlists),
            idx_list_split
        )
        p.close()
        p.join()
        ...
        logger.debug(
            f'Avg/min/max cell pop: {avg_cell_pop:>8.3f}/{min_cell_pop:>8d}/{max_cell_pop:>8d}'
        )
    ...
    def make_memberlists(self, cdf):
        ...
        rl = np.array(
            [len(self.memberlists[i]) for i in range(self.cellndx.shape[0])]
        )
        assert int(rl.sum()
                  ) == rdf.shape[0]    # check to make sure all atoms are counted
    ...
    def neighbors_of_cellndx(self, Ci):
        ...
        :rtype: _type_
        """
        assert self.cellndx_in_structure(
            Ci
    ...
    def are_cellndx_neighbors(self, Ci, Cj):
        ...
        assert self.cellndx_in_structure(
            Cj
        ), f'Error: cell {Cj} outside of cell structure {self.ncells}'
        dij = Ci - Cj
