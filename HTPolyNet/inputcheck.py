...
def input_check(args):
    ...
    for mname, M in C.molecules.items():
        ...
        if os.path.exists(os.path.join(lib, 'parameterized', f'{mname}.top')):
            M.TopoCoord = TopoCoord(
                grofilename=os.path.join(lib, 'parameterized', f'{mname}.gro'),
                topfilename=os.path.join(lib, 'parameterized', f'{mname}.top')
            )
            ...
            c = Coordinates.read_mol2(
                os.path.join(lib, 'inputs', f'{mname}.mol2')
            )
            matoms = c.A.shape[0]
        elif os.path.exists(os.path.join(lib, 'inputs', f'{mname}.pdb')):
            # print(os.path.join(lib,f"{mname}.{fmt}"))
            out, err = Command(
                f'grep -c ^ATOM {os.path.join(lib,"inputs",f"{mname}.pdb")}'
            ).run(ignore_codes=[1])
            matoms = int(out)
            out, err = Command(
                f'grep -c ^HETATM {os.path.join(lib,"inputs",f"{mname}.pdb")}'
            ).run(ignore_codes=[1])
            matoms += int(out)
