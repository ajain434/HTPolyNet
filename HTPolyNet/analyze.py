...
class Analyze:
    allowed_keys = [
        'gromacs', 'command', 'subdir', 'options', 'links', 'outfile',
        'console-input', 'matchlines'
    ]
    ...
    def do(self, **gromacs_dict):
        ...
        if 'console-input' in p:
            ...
            with open(cfile, 'w') as f:
                for ch in ci:
                    f.write(ch + '\n')
        self.console_output = gmx_command(
            p['command'], p.get('options', {}), console_in=cfile
        )
...
class AnalyzeDensity(Analyze):
    ...
    default_params = {
        ...
        'links':
            [
                'postsim/equilibrate/equilibrate.tpr',
                'postsim/equilibrate/equilibrate.trr'
            ],
        'gromacs': {
            'gmx': 'gmx'
        },
...
class AnalyzeFFV(Analyze):
    default_params = {
        ...
        'matchlines':
            [
                'Free volume', 'Total volume', 'Number of molecules',
                'Average molar mass', 'Density',
                'Molecular volume Vm assuming homogeneity:',
                'Molecular van der Waals volume assuming homogeneity:',
        ...
        for content in self.baselist:
            ...
            if analysistype in self.predefined_classes:
                self.stagelist.append(
                    self.predefined_classes[analysistype](content)
                )
            else:
...
def analyze(args):
    ...
    for d in args.proj:
        pfs.pfs_setup(
            root=os.getcwd(),
            topdirs=['molecules', 'systems', 'plots', 'postsim', 'analyze'],
            verbose=True,
            projdir=d,
            reProject=False,
