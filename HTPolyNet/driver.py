...
def run(args):
    ...
    pfs.pfs_setup(
        root=os.getcwd(),
        topdirs=['molecules', 'systems', 'plots'],
        verbose=True,
        projdir=args.proj,
        reProject=args.restart,
        userlibrary=userlib
    )
    ...
    if nm.isdecimal():
        ...
        if nm == 'all':
            for fullname in possibles:
                shutil.copy(
                    os.path.join(
                        l.get_example_depot_location(), f'{fullname}.tgz'
                    ), '.'
                )
...
def pack_example(args):
    ...
    os.chdir('..')
    c = Command(
        f'tar --exclude="*/*/*/*/*" -zvcf {depot_location}/{newname}.tgz {bn}/README.md {bn}/run.sh {bn}/*.yaml {bn}/lib/molecules/'
    )
    o, e = c.run()
    print(o)
    print(e)
...
def cli():
    ...
    helps[
        'run'
    ] = 'build a system using instructions in the config file and any required molecular structure inputs'
    helps[
        'parameterize'
    ] = 'parameterize monomers and oligomer templates using instructions in the config file'
    helps['info'] = 'print some information to the console'
    helps[
        'plots'
    ] = 'generate some plots that summarize aspects of the current completed build'
    helps[
        'fetch-example'
    ] = 'fetch and unpack example(s) from HTPolyNet.HTPolyNet.resources: ' + ', '.join(
        [f'"{x}"' for x in l.get_example_names()]
    )
    helps['pack-example'
         ] = 'pack current directory as a tarball to the resources/example_depot'
    helps[
        'input-check'
    ] = 'reports number of atoms that would be in initial system based on config'
    helps[
        'postsim'
    ] = 'perform specified postcure MD simulations on final results in one or more project directory'
    helps['analyze'
         ] = 'perform \'gmx <command>\' style analyses specified in the cfg file'
    parser = ap.ArgumentParser(
        description=textwrap.dedent(banner_message),
    ...
    ######## run ########
    command_parsers['run'].add_argument(
        'config',
        type=str,
        default=None,
    ...
    command_parsers['run'].add_argument(
        ...
        type=str,
        default='lib',
        help='local user library of molecular structures and parameterizations'
    )
    command_parsers['run'].add_argument(
        '-proj',
        type=str,
        default='next',
        help=
        'project directory; "next" (default) generates next directory.  Anything other than "next": if it exists, "-restart" must be included as a parameter; if not, it is created as a new project'
    )
    command_parsers['run'].add_argument(
        '-diag',
        type=str,
        default='htpolynet_runtime_diagnostics.log',
        help='diagnostic log file'
    )
    command_parsers['run'].add_argument(
        '-restart',
        default=False,
        action='store_true',
        help='restart in latest proj dir'
    )
    command_parsers['run'].add_argument(
        '--no-banner',
        default=False,
        action='store_true',
        help='turn off the banner'
    )
    command_parsers['run'].add_argument(
        '--force-parameterization',
        default=False,
        action='store_true',
        help='force GAFF parameterization of any input mol2 structures'
    )
    command_parsers['run'].add_argument(
        '--force-checkin',
        default=False,
        action='store_true',
        help=
        'force check-in of any generated parameter files to the system library'
    )
    command_parsers['run'].add_argument(
        '--loglevel',
        type=str,
        default='debug',
        help='Log level for messages written to diagnostic log (debug|info)'
    )
    ######## parameterize ########
    command_parsers['parameterize'].add_argument(
        'config',
        type=str,
        default=None,
        help='input configuration file in YAML format'
    ...
    ######## plots ########
    command_parsers['plots'].add_argument(
        'source',
        type=str,
        choices=['diag', 'build', 'post'],
        default='build',
        help='source of data to plot'
    )
    command_parsers['plots'].add_argument(
        '--diags',
        type=str,
        default=[],
        nargs='+',
        help='names of diagnostic log files (1 or more)'
    )
    command_parsers['plots'].add_argument(
        '--proj',
        nargs='+',
        type=str,
        default=[],
        help='name of project director[y/ies]'
    )
    command_parsers['plots'].add_argument(
        '--cfg', type=str, nargs='+', default=[], help='name input config files'
    )
    command_parsers['plots'].add_argument(
        '--buildplot',
        type=str,
        ...
        choices=['t', 'g', 'n', 'c'],
        help=
        'type of build plot to generate: t: traces (select using --traces); g: 2-D graph representations iteration by iteration; n: homo-N between crosslinks; c: cluster-size distributions'
    )
    command_parsers['plots'].add_argument(
        '--traces',
        type=str,
    ...
    command_parsers['plots'].add_argument(
        '--plotfile', type=str, default='', help='name of plot file to generate'
    )
    command_parsers['plots'].add_argument(
        '--no-banner',
        default=False,
    ...
    ######## fetch-example ########
    command_parsers['fetch-example'].add_argument(
        '-n',
        type=str,
        choices=example_ids + ['all'],
        help='number of example tarball to unpack from ' +
        ', '.join(example_names)
    )
    command_parsers['fetch-example'].add_argument(
        '-k', default=False, action='store_true', help='keep tarballs'
    )
    ######## pack-example
    command_parsers['pack-example'].add_argument(
        '-n',
        type=int,
        default=-1,
        help='desired index (integer) for this example'
    )
    ######## input-check ########
    command_parsers['input-check'].add_argument(
        'config',
        type=str,
        default=None,
        help='input configuration file in YAML format'
    ...
    ######## postsim ########
    command_parsers['postsim'].add_argument(
        '-proj',
        type=str,
        default='',
        nargs='+',
    ...
    command_parsers['postsim'].add_argument(
        ...
        type=str,
        default='',
        help='original HTPolyNet config file used to generate project(s)'
    )
    command_parsers['postsim'].add_argument(
        '-cfg',
        type=str,
        default='',
        help='config file for specifying the MD simulations to perform'
    )
    command_parsers['postsim'].add_argument(
        '--no-banner',
        default=False,
    ...
    ######## analyze ########
    command_parsers['analyze'].add_argument(
        '-proj',
        type=str,
        default='',
        nargs='+',
    ...
    command_parsers['analyze'].add_argument(
        ...
        type=str,
        default='',
        help='config file for specifying the analyses to perform'
    )
    command_parsers['analyze'].add_argument(
        '--no-banner',
        default=False,
