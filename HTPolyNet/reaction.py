...
class Reaction:
    ...
    def __init__(self, jsondict={}):
        ...
        self.reactants = jsondict.get(
            'reactants', self.default_directives['reactants']
        )
        self.product = jsondict.get(
            'product', self.default_directives['product']
        )
        self.stage = reaction_stage[jsondict.get(
            'stage', str(self.default_directives['stage'])
        )]
        self.procession = jsondict.get(
            'procession', self.default_directives['procession']
        )
        self.probability = jsondict.get(
            'probability', self.default_directives['probability']
        )
...
def parse_reaction_list(baselist: ReactionList):
    ...
    for i, R in enumerate(baselist):
        if R.procession:
            ...
            for c in range(R.procession['count']):
                ...
                nR.product = f'{final_product_name}_I{c+1}' if c < (
                    R.procession['count'] - 1
                ) else final_product_name
                nR.reactants[base_reactant_key] = f'{final_product_name}_I{c}'
                for ak, arec in nR.atoms.items():
                    if arec['reactant'] == base_reactant_key:
                        arec['resid'] += (c + 1)
                nR.procession = {}
    ...
    for rec in processives:
        atidx, sublist = rec
        atidx += lasti
        ret_reactions = ret_reactions[0:atidx +
                                      1] + sublist + ret_reactions[atidx + 1:]
        lasti += len(sublist)
...
def extract_molecule_reactions(rlist: ReactionList, plot=True):
    ...
    if plot:
        network_graph(
            G,
            'plots/reaction_network.png',
            arrows=True,
            with_labels=True,
            figsize=(10, 10),
            node_size=0
        )
    input_reactants = reactants.intersection(
        reactants.symmetric_difference(products)
    )
...
def is_reactant(
    name: str, reaction_list: ReactionList, stage=reaction_stage.cure
):
...
def product_sequence_resnames(R: Reaction, reactions: ReactionList):
    ...
    for rKey, rName in R.reactants.items():
        if rName in allProducts:
            result.extend(
                product_sequence_resnames(
                    reactions[allProducts.index(rName)], reactions
                )
            )
        else:
            result.extend([rName])
...
def molname_sequence_resnames(molname: str, reactions: ReactionList):
    ...
    if molname in prodnames:
        ...
        return [
            molname
        ]    # not a product in the reaction list?  must be a monomer


def reactant_resid_to_presid(
    R: Reaction, reactantName: str, resid: int, reactions: ReactionList
):
    ...
    if reactantName in reactants:
        ...
        logger.debug(
            f'reactantName {reactantName} {ridx} in {reactants} of {R.name}'
        )
        S = 0
        for i in range(ridx):
