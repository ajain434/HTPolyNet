...
def repeat_check(t, msg=''):
    ...
    for i in range(len(t)):
        for j in range(i + 1, len(t)):
            assert t[i] != t[
                j
            ], f'Error: repeated index in {len(t)}-tuple {t}: t({i})={t[i]}\n{msg}'
...
_GromacsIntegers_ = (
    'nr', 'atnum', 'resnr', 'ai', 'aj', 'ak', 'al', '#mols', 'nrexcl', 'funct',
    'func', 'nbfunc', 'comb-rule'
)
_GromacsFloats_ = (
    'charge', 'mass', 'chargeB', 'massB', *tuple([f'c{i}'
                                                  for i in range(5)]), 'b0',
    'kb', 'th0', 'cth', 'rub', 'kub', 'phase', 'kd', 'pn', 'fudgeLJ', 'fudgeQQ'
)
...
_GromacsTopologyDirectiveOrder_ = [
    'defaults', 'atomtypes', 'bondtypes', 'angletypes', 'dihedraltypes',
    'moleculetype', 'atoms', 'pairs', 'bonds', 'angles', 'dihedrals', 'system',
    'molecules'
]
...
def select_topology_type_option(
    options, typename='dihedraltypes', rule='stiffest'
):
    ...
    def read_gro(cls, filename):
        ...
        with open(filename, 'r') as f:
            ...
            for s in stanzas:
                ...
                contentlines = [
                    l for l in s[1:]
                    if not (len(l.strip()) == 0 or l.strip().startswith(';'))
                ]
                if not directive in _GromacsTopologyDirectiveHeaders_:
                    raise KeyError(
                        f'unrecognized topology directive "{directive}"'
                    )
                ...
                for line in contentlines:
                    ...
                    assert len(padded) == len(
                        header
                    ), f'Error: Padding solution does not work! {directive} {len(tokens)}:{len(padded)}!={len(header)} {",".join(tokens)} {",".join(header)}'
                ...
                if directive == 'dihedraltypes':
                    if directive in inst.D:
                        ...
                        inst.D['dihedraltypes'] = pd.concat(
                            [inst.D['dihedraltypes'], tdf], ignore_index=True
                        ).drop_duplicates()
                    ...
                    if directive in inst.D:
                        inst.D['dihedrals'] = pd.concat(
                            [inst.D['dihedrals'], tdf], ignore_index=True
                        )
                    else:
                        inst.D[directive] = tdf
                else:
            ...
            if 'atomtypes' in inst.D:
                inst.D['atomtypes'] = inst.D['atomtypes'].sort_values(
                    by='name'
                ).reset_index(drop=True)
#                inst.D['atomtypes'].sort_values(by='name',inplace=True)
            if 'bonds' in inst.D:
                inst.D['bonds'] = inst.D['bonds'].sort_values(
                    by=['ai', 'aj']
                ).reset_index(drop=True)
            ...
            if 'dihedraltypes' in inst.D:
                ...
                inst.D['dihedraltypes'] = inst.D['dihedraltypes'].sort_values(
                    by=['j', 'k', 'i', 'l']
                ).reset_index(drop=True)
        ...
        if count > 0:
            ...
            for c in range(1, count):
                ...
                self.shiftatomsidx(
                    idxshift * c,
                    'atoms',
                    rows=[(c * counts['atoms']), ((c + 1) * counts['atoms'])],
                    idxlabels=['nr']
                )
                self.shiftatomsidx(
                    c,
                ...
                self.shiftatomsidx(
                    idxshift * c,
                    'bonds',
                    rows=[(c * counts['bonds']), ((c + 1) * counts['bonds'])],
                    idxlabels=['ai', 'aj']
                )
                self.shiftatomsidx(
                    idxshift * c,
                ...
                self.shiftatomsidx(
                    idxshift * c,
                    'pairs',
                    rows=[(c * counts['pairs']), ((c + 1) * counts['pairs'])],
                    idxlabels=['ai', 'aj']
                )
                self.shiftatomsidx(
                    idxshift * c,
                    'angles',
                    rows=[(c * counts['angles']), ((c + 1) * counts['angles'])],
                    idxlabels=['ai', 'aj', 'ak']
                )
                self.shiftatomsidx(
                    idxshift * c,
                    ...
                    rows=[
                        (c * counts['dihedrals']),
                        ((c + 1) * counts['dihedrals'])
                    ],
                    idxlabels=['ai', 'aj', 'ak', 'al']
                )
            if 'bonds' in self.D:
    ...
    def adjust_charges(
        self, atoms=[], desired_charge=0.0, overcharge_threshhold=0.1, msg=''
    ):
        ...
        logger.debug(
            f'Adjusting charges of {len(atoms)} atoms due to overcharge of {overcharge:.6f}'
        )
        ...
        logger.debug(
            f'New total charge after adjustment: {self.total_charge():.6f}'
        )
        return self
    ...
    def add_restraints(self, pairdf, typ=6, kb=300000.):
        ...
        for i, b in pairdf.iterrows():
            ...
            if not (ai, aj) in bmi:
                ...
                data = [
                    ai, aj, typ, b0, kb
                ]    # this new bond will have override parameters
        ...
        for b in pairs:
            ...
            if not (ai, aj) in bmi:
                ...
                if idx in ij.index:
                    ...
                    logger.debug(
                        f'no bondtype {idx} found; using placeholder parameters'
                    )
                    bt = 1
                    b0 = 0.15
                    kb = 999999
                ...
                if 'mol2_bonds' in self.D:
                    data = [
                        len(self.D['mol2_bonds']), ai, aj, 1
                    ]    # assume single bond
                    bonddict = {
                        k: [v]
                        for k, v in zip(['bondIdx', 'ai', 'aj', 'order'], data)
                    }
                    self.D['mol2_bonds'] = pd.concat(
                        (self.D['mol2_bonds'], pd.DataFrame(bonddict)),
                        ignore_index=True
                    )
                # remove this pair from pairs if it's in there (it won't be)
                if idx in pmi:
                    logger.debug(
                        f'Warning: new bond {ai}-{aj} was evidently in the [ pairs ]!'
                    )
                    d = self.D['pairs']
                    indexes_to_drop = d[(d.ai.isin(idx)) &
                                        (d.aj.isin(idx))].index
                    logger.debug(
                        f'Dropping [ pair ]:\n{d[indexes_to_drop].to_string()}'
                    )
                    indexes_to_keep = set(range(d.shape[0])
                                         ) - set(indexes_to_drop)
                    self.D['pairs'] = d.take(list(indexes_to_keep)
                                            ).reset_index(drop=True)
            else:
                ''' 
    ...
    def delete_atoms(self, idx=[], reindex=True, return_idx_of=[], **kwargs):
        ...
        logger.debug(
            f'Deleting {d.loc[indexes_to_drop].shape[0]} [ atoms ]; charge to make up: {total_missing_charge:.4f}'
        ...
        if reindex:
            ...
            assert not any(
                [x in mapper for x in idx]
            ), f'Error: Some deleted atoms in mapper.'
        ...
        for pt in ptt:
            if pt in self.D:
                ...
                logger.debug(
                    f'Deleting {d.loc[indexes_to_drop].shape[0]} [ {pt} ]'
                )
                indexes_to_keep = set(range(d.shape[0])) - set(indexes_to_drop)
                ...
                if reindex:
                    d = self.D[pt]
                    assert not any(
                        [x in idx for x in d.ai]
                    ), f'Error: deleted atom survived in {pt} ai'
                    assert not any(
                        [x in idx for x in d.aj]
                    ), f'Error: deleted atom survived in {pt} aj'
                    assert all(
                        [x in mapper for x in d.ai]
                    ), f'Error: surviving {pt} atom ai old idx not in mapper'
                    assert all(
                        [x in mapper for x in d.aj]
                    ), f'Error: surviving {pt} atom aj old idx not in mapper'
        ...
        indexes_to_drop = d[(d['ai'].isin(idx)) | (d['aj'].isin(idx)) |
                            (d['ak'].isin(idx))].index
        ...
        if reindex:
            ...
            assert not any(
                [x in idx for x in d.ai]
            ), 'Error: deleted atom survived in angle ai'
            assert not any(
                [x in idx for x in d.aj]
            ), 'Error: deleted atom survived in angle aj'
            ...
            assert not any(
                zombie_tags
            ), 'Error: deleted atom survived in angle ak'
            assert all(
                [x in mapper for x in d.ai]
            ), 'Error: surviving angle atom ai old idx not in mapper'
            assert all(
                [x in mapper for x in d.aj]
            ), 'Error: surviving angle atom aj old idx not in mapper'
            assert all(
                [x in mapper for x in d.ak]
            ), 'Error: surviving angle atom ak old idx not in mapper'
            d.ai = d.ai.map(mapper)
            d.aj = d.aj.map(mapper)
        ...
        d = self.D['dihedrals']
        indexes_to_drop = d[(d.ai.isin(idx)) | (d.aj.isin(idx)) |
                            (d.ak.isin(idx)) | (d.al.isin(idx))].index
        ...
        if paranoid_about_pairs:
            dp = self.D['pairs']
            ddp = d.loc[indexes_to_drop
                       ]    # these are dihedrals marked for deletion
            # determine pairs deriving from these dihedrals and delete them!
            pai = ddp.ai.to_list()
            pal = ddp.al.to_list()
            dd = []
            for pi, pl in zip(pai, pal):
                dwpi = dp[((dp.ai == pi) & (dp.aj == pl)) |
                          ((dp.ai == pl) & (dp.aj == pi))].index.to_list()
                dd.extend(dwpi)
            ptk = set(range(dp.shape[0])) - set(dwpi)
            if dp.loc[dwpi].shape[0] > 0:
                logger.debug(
                    f'  -> and deleting {dp.loc[dwpi].shape[0]} [ pairs ] from those dihedrals'
                )
        ...
        indexes_to_keep = set(range(d.shape[0])) - set(indexes_to_drop)
        self.D['dihedrals'] = d.take(list(indexes_to_keep)
                                    ).reset_index(drop=True)
        if reindex:
            d = self.D['dihedrals']
            d.ai = d.ai.map(mapper)
            ...
            for i, r in d.iterrows():
                ...
                if not [ai, aj] in tp:
                    ...
                    pdrops.append(i)
            logger.debug(
                f'Deleting {len(pdrops)} duplicate 1-4 pair descriptors -- this is likely due to a bug somewhere'
            )
    ...
    def _myconcat(
        ...
        if directive in self.D:
            ...
            if drop_duplicates:
                self.D[directive] = pd.concat(
                    (self.D[directive], other.D[directive]), ignore_index=True
                ).drop_duplicates()
            else:
                self.D[directive] = pd.concat(
                    (self.D[directive], other.D[directive]), ignore_index=True
...
# #        pass
    ...
    def reset_override_from_type(self, interactionname, typename, inst_idx):
        ...
        logger.debug(
            f'Resetting override in {interactionname} for {inst_idx} from'
        )
        for ln in self.D[interactionname].iloc[idx,
                                               cols].to_string().split('\n'):
            logger.debug(ln)
        logger.debug('to')
    ...
    def reset_type(self, typename, typidx_t, values):
        ...
        for idx in idxs:
            ...
            for ln in self.D[typename].iloc[idx][headers].to_string(
            ).split('\n'):
                logger.debug(ln)
    ...
    def report_duplicate_types(self, other, typename='', funcidx=4):
        ...
        for idx, r in otdf.iterrows():
            ...
            for jdx, q in stdf.iterrows():
                typjdx = typeorder(tuple([q[i] for i in hashables]))
                jdata = [q[i] for i in headers]
                if typidx == typjdx and idata != jdata and not typidx in common and idata[
                    fi] == funcidx and jdata[fi] == funcidx:
                    # logger.debug(f'duplicate {typename} {typidx}')
    ...
    def dup_check(self, die=True):
        ...
        for t in L:
            ...
            if any(dups):
                logger.error(
                    f'Duplicate {t} with different parameters detected\n' +
                    self.D[t][dups].to_string()
                )
                if die:
                    raise Exception(
                        'duplicate topology types with different parameters detected'
                    )
    ...
    def merge_ex(self, other):
        ...
        rdxshift = 0 if 'atoms' not in self.D else self.D['atoms'].iloc[-1][
            'resnr']
        if 'atoms' in other.D:
            other.D['atoms']['resnr'] += rdxshift
        self._myconcat(
            other, directive='atoms', idxlabel=['nr'], idxshift=idxshift
        )
        self._myconcat(
            other, directive='bonds', idxlabel=['ai', 'aj'], idxshift=idxshift
        )
        if 'bonds' in other.D:
        ...
        self._myconcat(
            other,
            directive='mol2_bonds',
            idxlabel=['ai', 'aj'],
            idxshift=idxshift
        )
        self._myconcat(
            other, directive='pairs', idxlabel=['ai', 'aj'], idxshift=idxshift
        )
        self._myconcat(
            other,
            directive='angles',
    ...
    def make_resid_graph(self, json_file=None):
        ...
        if json_file:
            ...
            assert type(
                the_data
            ) == dict, f'Error: node_link_data returns a {type(the_data)} but should return a dict'
            ...
            if "'" in the_data_str:
                logger.debug(
                    f'json_graph.node_link_data produces single-quoted dict keys -- this is not JSON standard'
                )
                ...
                try:
                    ...
                    logger.debug(
                        f'json.loads fails to encode string:\n{json_compatible_string}'
                    )
            with open(json_file, 'w') as f:
                try:
                    ...
                    logger.debug(str(msg))
                    logger.debug(
                        f'writing resid graph to JSON not currently supported'
                    )
    ...
    def attenuate_bond_parameters(
        ...
        logger.debug(
            f'Attenuating {bondsdf.shape[0]} bond{"s" if bondsdf.shape[0]>1 else ""} in stage {stage+1}/{max_stages}'
        )
    ...
    def attenuate_pair_parameters(
        self, pairsdf, stage, max_stages, draglimit_nm=0.3
    ):
        ...
        factor = (stage + 1) / max_stages
        logger.debug(
            f'Attenuating {pairsdf.shape[0]} pair{ess} in stage {stage+1}/{max_stages}'
        )
        for b in pairsdf.itertuples():
            ai, aj = idxorder((b.ai, b.aj))
            b0 = b.initial_distance
            kb = pdf.loc[(pdf['ai'] == ai) & (pdf['aj'] == aj), 'c1'].values[0]
            pdf.loc[(pdf['ai'] == ai) & (pdf['aj'] == aj),
                    'c0'] = draglimit_nm - factor * (b0 - draglimit_nm)
