...
def GAFFParameterize(
    ...
    logger.info(
        f'AmberTools> generating GAFF parameters from {inputPrefix}.{input_structure_format}'
    )
    ...
    if structin == mol2out:
        ...
        logger.debug(
            f'AmberTools> Antechamber overwrites input {structin}; backing up to {new_structin}'
        )
    ...
    leapprefix = hashlib.shake_128(outputPrefix.encode("utf-8")
                                  ).hexdigest(16).replace('e', 'x')
    goodMol2.write_mol2(f'{leapprefix}.mol2')
    logger.debug(
        f'Replacing string "{outputPrefix}" with hash "{leapprefix}" for leap input files.'
    )
    ...
    try:
        file = parmed.load_file(
            f'{outputPrefix}-tleap.top', xyz=f'{outputPrefix}-tleap.crd'
        )
