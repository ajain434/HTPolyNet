...
class Software:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(
                'It seems like you do not have an accessible installation of ambertools.'
            )
    ...
    def set_gmx_preferences(self, parameters):
        ...
        if gromacs_dict:
            ...
            self.mdrun = gromacs_dict.get(
                'mdrun', f'{self.gmx} {self.gmx_options} mdrun'
            )
            self.mdrun_single_molecule = gromacs_dict.get(
                'mdrun_single_molecule', f'{self.gmx} {self.gmx_options}  mdrun'
            )
            ...
            self.mdrun = parameters.get(
                'mdrun', f'{self.gmx} {self.gmx_options} mdrun'
            )
            self.mdrun_single_molecule = parameters.get(
                'mdrun_single_molecule', f'{self.gmx} {self.gmx_options}  mdrun'
            )
    ...
    def __str__(self):
        ...
        for c in self.ambertools:
            r.append(
                f'{os.path.split(c)[1]:>12s} ({self.versions["ambertools"]:>s})'
            )
        return '\n'.join(r)
    ...
    def getVersions(self):
        ...
        if self.passes:
            CP = subprocess.run(
                ['antechamber', '-h'], capture_output=True, text=True
            )
